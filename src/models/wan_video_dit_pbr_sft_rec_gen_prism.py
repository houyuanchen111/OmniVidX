...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, drop_out, training_mode: str = None): # 默认不dropout

        # 先不停梯度试试看
        if training_mode == "RGBX_Generation" or training_mode == "RGB2X":
            q = self.q(x, adapter_names=[None,"albedo", "irradiance", "normal"]) # 这样的写法是可以的
            ...
            q = self.q(x, adapter_names=["relight","albedo", "irradiance", "normal"]) # 这样的写法是可以的
            k = self.k(x, adapter_names=["relight","albedo", "irradiance", "normal"])
            v = self.v(x, adapter_names=["relight","albedo", "irradiance", "normal"])
            q = self.norm_q(q)
            k = self.norm_k(k)
            q = rope_apply(q, freqs, self.num_heads)
            k = rope_apply(k, freqs, self.num_heads)
            x = self.attn(q, k, v, drop_out=drop_out)
            result = self.o(x, adapter_names=["relight","albedo", "irradiance", "normal"])    
        return result
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, drop_out: int = 0.25, training_mode: str = None):
