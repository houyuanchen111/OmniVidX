...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs):
        ...
        # 合并到序列维度进行自注意力
        x_merged = torch.cat([x_albedo, x_material, x_normal], dim=1)        # (1, 3L, C)
        input_x_merged = torch.cat([input_x_albedo, input_x_material, input_x_normal], dim=1)

        # RoPE频率对齐到3L长度
        freqs_3L = torch.cat([freqs, freqs, freqs], dim=0)
        ...
        x = torch.cat([x_albedo_seq, x_material_seq, x_normal_seq], dim=0)
...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        # 可学习的 domain embedding，用于区分不同模态（例如 albedo / material / normal）
