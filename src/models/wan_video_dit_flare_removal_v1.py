# condition的时候不加lora，以及把inpainting任务去掉
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, drop_out: float = 0.25, training_mode: str = None): # drop_out=0.25
        if training_mode == "Generation":
            q = self.q(x, adapter_names=["com", "flare", "clean"])
            k = self.k(x, adapter_names=["com", "flare", "clean"])
            v = self.v(x, adapter_names=["com", "flare", "clean"])
            q = self.norm_q(q)
            k = self.norm_k(k)
            q = rope_apply(q, freqs, self.num_heads)
            k = rope_apply(k, freqs, self.num_heads)
            x = self.attn(q, k, v, drop_out=drop_out)
            result = self.o(x, adapter_names=["com", "flare", "clean"])
        elif training_mode == "FlareRemoval":
            q = self.q(x, adapter_names=[None, "flare", "clean"])
            k = self.k(x, adapter_names=[None, "flare", "clean"])
            v = self.v(x, adapter_names=[None, "flare", "clean"])
            q = self.norm_q(q)
            k = self.norm_k(k)
            q = rope_apply(q, freqs, self.num_heads)
            k = rope_apply(k, freqs, self.num_heads)
            x = self.attn(q, k, v, drop_out=drop_out)
            result = self.o(x, adapter_names=[None, "flare", "clean"])
        elif training_mode == "Composition":
            q = self.q(x, adapter_names=["com", None, None])
            k = self.k(x, adapter_names=["com", None, None])
            v = self.v(x, adapter_names=["com", None, None])
            q = self.norm_q(q)
            k = self.norm_k(k)
            q = rope_apply(q, freqs, self.num_heads)
            k = rope_apply(k, freqs, self.num_heads)
            x = self.attn(q, k, v, drop_out=drop_out)
            result = self.o(x, adapter_names=["com", None, None])
        else:
            print("wrong!!!!")
        return result
