...
def rope_apply(x, freqs, num_heads):
    x = rearrange(x, "b s (n d) -> 1 (b s) n d", n=num_heads)
    x_out = torch.view_as_complex(x.to(torch.float64).reshape(
        x.shape[0], x.shape[1], x.shape[2], -1, 2))
    x_out = torch.view_as_real(x_out * freqs).flatten(2)
    x_out = rearrange(x_out, "1 (b s) c -> b s c", b=4)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, drop_out: float = 0, training_mode: str = None): # drop_out=0.25
        if training_mode == "PBRGeneration":
            ...
            print("training_mode == Relighting")
            q = self.q(x)
            k = self.k(x)
            v = self.v(x)
            q = self.norm_q(q)
            ...
            print("training_mode == Reconstruction")
            q = self.q(x)
            k = self.k(x)
            v = self.v(x)
            q = self.norm_q(q)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, drop_out: float = 0):
        # 时间统计 - 可以注释掉
        # import time
