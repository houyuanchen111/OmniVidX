...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, drop_out): # 默认不dropout
        if x.shape[0] == 4:
            q = self.q(x, adapter_names=[None,"albedo", "material", "normal"]) # 这样就不用加rgb lora
            k = self.k(x, adapter_names=[None,"albedo", "material", "normal"])
            v = self.v(x, adapter_names=[None,"albedo", "material", "normal"])
            q = self.norm_q(q)
            k = self.norm_k(k)
            ...
            result = self.o(x, adapter_names=[None,"albedo", "material", "normal"])    
        elif x.shape[0] == 3:
            print("x.shape[0] == 3")
            q = self.q(x, adapter_names=["albedo", "material", "normal"]) # 这样的写法是可以的
            k = self.k(x, adapter_names=["albedo", "material", "normal"])
            v = self.v(x, adapter_names=["albedo", "material", "normal"])
            q = self.norm_q(q)
            k = self.norm_k(k)
            q = rope_apply(q, freqs, self.num_heads)
            k = rope_apply(k, freqs, self.num_heads)
            x = self.attn(q, k, v, drop_out=drop_out)
            result = self.o(x, adapter_names=["albedo", "material", "normal"])    
        else:
            print("x.shape[0] != 4 and x.shape[0] != 3")
        return result
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, drop_out: int = 1):
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, drop_out: int = 0.25):
        ...
        x = self.gate(x, gate_msa, self.self_attn(input_x, freqs, drop_out=drop_out)) # self_attn
