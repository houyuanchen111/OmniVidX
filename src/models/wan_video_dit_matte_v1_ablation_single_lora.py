...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, drop_out: float = 0.25, training_mode: str = None): # drop_out=0.25
        ...
        #     q = self.q(x, adapter_names=["com", None, "fgr", None])
        #     k = self.k(x, adapter_names=["com", None, "fgr", None])
        #     v = self.v(x, adapter_names=["com", None, "fgr", None])
        #     q = self.norm_q(q)
        #     k = self.norm_k(k)
        #     q = rope_apply(q, freqs, self.num_heads)
        #     k = rope_apply(k, freqs, self.num_heads)
        #     x = self.attn(q, k, v, drop_out=drop_out)
        #     result = self.o(x, adapter_names=["com", None, "fgr", None])
