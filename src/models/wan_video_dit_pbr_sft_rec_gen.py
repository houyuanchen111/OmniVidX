...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, drop_out): # 默认不dropout
        if x.shape[0] == 4:
            q = self.q(x, adapter_names=["rgb","albedo", "material", "normal"]) # 这样的写法是可以的
            k = self.k(x, adapter_names=["rgb","albedo", "material", "normal"])
            v = self.v(x, adapter_names=["rgb","albedo", "material", "normal"])
            q = self.norm_q(q)
            k = self.norm_k(k)
            q = rope_apply(q, freqs, self.num_heads)
            k = rope_apply(k, freqs, self.num_heads)
            x = self.attn(q, k, v, drop_out=drop_out)
            result = self.o(x, adapter_names=["rgb","albedo", "material", "normal"])    
        elif x.shape[0] == 3:
            print("x.shape[0] == 3")
