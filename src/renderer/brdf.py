...
def smithG_GGX(ndv, a):
    a = a * a
    b = ndv * ndv
    return 1.0 / (ndv + sqrt_(a + b - a * b) + 1e-8)
...
def eval_disney(albedo: torch.Tensor, roughness: torch.Tensor, metallic: torch.Tensor, wi: torch.Tensor,
    ...
    # print(f"Fs: {Fs}")
    # print(f"ndh: {ndh}")
    mask = torch.where(ndl < 0, torch.zeros_like(ndl), torch.ones_like(ndl))
...
def pdf_diffuse(wi: torch.Tensor, wo: torch.Tensor):
    ...
    """
    pdf = wo[:,:,2:3,...] / np.pi
    pdf = torch.where(wi[:,:,2:3,...] <= 0, torch.zeros_like(pdf), pdf)
    pdf = torch.where(wo[:,:,2:3,...] <= 0, torch.zeros_like(pdf), pdf)
    return pdf
