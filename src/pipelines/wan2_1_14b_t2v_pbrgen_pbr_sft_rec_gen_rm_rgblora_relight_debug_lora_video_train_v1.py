...
class WanVideoPipeline(BasePipeline):
    ...
    def enable_usp(self):
        ...
        from ..distributed.xdit_context_parallel_rec_gen_rm_rgblora import usp_attn_forward, usp_dit_forward
...
class WanTrainingModule_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_rm_rgblora_relight_debug_lora_video_train_v1(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        self.pipe = WanVideoPipeline.from_pretrained(torch_dtype=torch.bfloat16, device=device, tokenizer_config=tokenizer_config, model_configs=model_configs,redirect_common_files=False,use_usp=False) # 加载预训练权重
        ...
        if lora_base_model is not None:
            ...
            for name, param in self.pipe.dit.named_parameters():
                if "lora" in name.lower() and "normal" not in name.lower() and "albedo" not in name.lower(): # 不放开rgb的LoRA,让其保证只使用原因模型的权重
