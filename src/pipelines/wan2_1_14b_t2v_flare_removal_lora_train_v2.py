"""
flare removal 任务, 每个branch都有对应的prompt
""" 
import torchvision
...
from ..models.wan_video_dit_flare_removal_v1 import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        # 三个分支: [com, flare, clean]
        if training_mode == "Composition":
            # 预测 com，使用 clean 与 flare 作为条件（t=999）
            timestep = [
                timestep_weight,
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
            ]
        elif training_mode == "FlareRemoval":
            # 预测 clean 和 flare，使用 com 作为条件（t=999）
            timestep = [
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                timestep_weight,
                timestep_weight,
            ]
        elif training_mode == "Generation":
            # 从纯噪声生成所有三个分支
            timestep = [timestep_weight, timestep_weight, timestep_weight]
        else:
            # 兜底：全部使用相同时间步
            timestep = [timestep_weight, timestep_weight, timestep_weight]
    ...
    def __call__(
        ...
        **kwargs 
        ...
        # 三分支的推理初始化: [com(0), flare(1), clean(2)]
        if training_mode == "Composition":
            # Composition: 预测 com，条件为 clean 和 flare
            if "inference_flare_latents" in inputs_shared:
                inputs_shared["latents"][1], _ = self.scheduler.add_noise(inputs_shared["inference_flare_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device))
            if "inference_clean_latents" in inputs_shared:
                inputs_shared["latents"][2], _ = self.scheduler.add_noise(inputs_shared["inference_clean_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device))
        elif training_mode == "FlareRemoval":
            # FlareRemoval: 预测 flare 和 clean，条件为 com
            if "inference_com_latents" in inputs_shared:
                inputs_shared["latents"][0], _ = self.scheduler.add_noise(inputs_shared["inference_com_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device))
        # Generation: 从纯噪声生成，无需额外初始化

        for progress_id, timestep in enumerate(tqdm(scheduler_copy.timesteps)):
            timestep = timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device)
            # 推理timestep设置: [com, flare, clean]
            if training_mode == "Composition":
                # Composition: 只去噪 com
                timestep = [
                    timestep,
                ...
                # FlareRemoval: 只去噪 flare 和 clean
                timestep = [
                    self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                    timestep,
                    timestep,
            ...
            # Scheduler - 三分支的latents更新
            step_latents = scheduler_copy.step(noise_pred, scheduler_copy.timesteps[progress_id], inputs_shared["latents"]) 
            if training_mode == "Composition":
                # 只更新 com (索引0)
                inputs_shared["latents"][[0]] = step_latents[[0]]
            elif training_mode == "FlareRemoval":
                # 更新 flare (索引1) 和 clean (索引2)
                inputs_shared["latents"][[1]] = step_latents[[1]]
                inputs_shared["latents"][[2]] = step_latents[[2]]
            elif training_mode == "Generation":
                # 更新所有三个分支
        ...
        # Decode - 三分支: [com, flare, clean]
        self.load_models_to_device(['vae'])
        video_dict = {}

        video_com = self.vae.decode(inputs_shared["latents"][ [0] ], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_com = self.vae_output_to_video(video_com, pattern="B C T H W")
        video_dict["com"] = video_com # torch.tensor[c,t,h,w]
        video_flare = self.vae.decode(inputs_shared["latents"][ [1] ], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_flare = self.vae_output_to_video(video_flare, pattern="B C T H W")
        video_dict["flare"] = video_flare # torch.tensor[c,t,h,w]
        video_clean = self.vae.decode(inputs_shared["latents"][ [2] ], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_clean = self.vae_output_to_video(video_clean, pattern="B C T H W")
        video_dict["clean"] = video_clean # torch.tensor[c,t,h,w]
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device):
        ...
        noise = pipe.generate_noise((3, 16, length, height//8, width//8), seed=seed, rand_device=rand_device) # 三个分支: [com, flare, clean]
        return {"noise": noise}
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def __init__(self):
        super().__init__(
            input_params=("input_videos", "noise", "tiled", "tile_size", "tile_stride", "is_inference", "modality_index", "inference_com", "inference_flare", "inference_clean", "height", "width", "training_mode"),
            onload_model_names=("vae",)
        )
    def process(self, pipe: WanVideoPipeline, input_videos, noise, tiled, tile_size, tile_stride, is_inference, modality_index, inference_com, inference_flare, inference_clean, height, width, training_mode):
        if is_inference:
            ...
            if training_mode == "Composition":
                # 条件：clean 与 flare
                if inference_clean is not None:
                    if isinstance(inference_clean, Image.Image):
                        inference_clean = pipe.preprocess_image(inference_clean.resize((width, height))).to(pipe.device).to(pipe.torch_dtype)[:,:,None]
                    elif isinstance(inference_clean, torch.Tensor):
                        inference_clean = pipe.preprocess_video(inference_clean)
                    inference_clean_latents = pipe.vae.encode(inference_clean, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
                    result["inference_clean_latents"] = inference_clean_latents
                if inference_flare is not None:
                    if isinstance(inference_flare, Image.Image):
                        inference_flare = pipe.preprocess_image(inference_flare.resize((width, height))).to(pipe.device).to(pipe.torch_dtype)[:,:,None]
                    elif isinstance(inference_flare, torch.Tensor):
                        inference_flare = pipe.preprocess_video(inference_flare)
                    inference_flare_latents = pipe.vae.encode(inference_flare, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
                    result["inference_flare_latents"] = inference_flare_latents
                return result
            elif training_mode == "FlareRemoval":
                # 条件：只有 com
                if inference_com is not None:
                    if isinstance(inference_com, Image.Image):
                        inference_com = pipe.preprocess_image(inference_com.resize((width, height))).to(pipe.device).to(pipe.torch_dtype)[:,:,None]
                    elif isinstance(inference_com, torch.Tensor):
                        inference_com = pipe.preprocess_video(inference_com)
                    inference_com_latents = pipe.vae.encode(inference_com, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
                    result["inference_com_latents"] = inference_com_latents
                return result
...
class WanTrainingModule_wan2_1_14b_t2v_flare_removal_lora_train_v2(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        if dist.is_available() and dist.is_initialized():
            ...
            if dist.get_rank() == 0:
                ...
                if flag < 1/3:
                    ...
                    mode_idx = 1  # "FlareRemoval"
                else:
                    mode_idx = 2  # "Composition"
            else:
            ...
            # 映射回字符串
            modes = ["Generation", "FlareRemoval", "Composition"]
    ...
    def forward_preprocess(self, data):
        ...
        if inputs_shared["training_mode"] == "Generation":
            prompt_list = [
                data["prompt"][0] + "Lighting effect: " + data["flare_prompt"][0],
                data["flare_prompt"][0],
                data["prompt"][0]
            ]
            inputs_posi = {"prompt": prompt_list}
        elif inputs_shared["training_mode"] == "FlareRemoval":  
            prompt_list = [
                "",
                data["flare_prompt"][0],
                data["prompt"][0]
            ...
            inputs_posi = {"prompt": prompt_list}
        elif inputs_shared["training_mode"] == "Composition":
            prompt_list = [
                data["prompt"][0] + "Lighting effect: " + data["flare_prompt"][0],
                "",
                ""
            ...
            inputs_posi = {"prompt": prompt_list}
        input_videos = torch.cat((data["com"], data["flare"], data["clean"]), dim=0) # 3b c t h w
