...
from ..models.wan_video_dit_nvs_v1 import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        # NVS任务的四个分支: [cam1, cam2, cam3, cam4]
        ...
        elif training_mode == "cam1":
            timestep = [
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                timestep_weight,
                timestep_weight,
                timestep_weight,
                ] # cam1作为条件: cam1 branch timestep为999 近似于不加噪，预测 cam2, cam3, cam4
        elif training_mode == "cam2":
            timestep = [
                timestep_weight,
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                timestep_weight,
                timestep_weight,
                ] # cam2作为条件: cam2 branch timestep为999，预测 cam1, cam3, cam4
        elif training_mode == "cam3":
            timestep = [
                timestep_weight,
                timestep_weight,
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                timestep_weight,
                ] # cam3作为条件: cam3 branch timestep为999，预测 cam1, cam2, cam4
        elif training_mode == "cam4":
            timestep = [
                timestep_weight,
                timestep_weight,
                timestep_weight,
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                ] # cam4作为条件: cam4 branch timestep为999，预测 cam1, cam2, cam3
    ...
    def __call__(
        ...
        **kwargs 
        ...
        # NVS任务的推理初始化: [cam1, cam2, cam3, cam4]
        if training_mode == "cam1":
            # cam1作为条件: cam1分支条件固定，预测 cam2, cam3, cam4
            inputs_shared["latents"][0], sigmas = self.scheduler.add_noise(inputs_shared["inference_cam1_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device))
        elif training_mode == "cam2":
            # cam2作为条件: cam2分支条件固定，预测 cam1, cam3, cam4
            inputs_shared["latents"][1], sigmas = self.scheduler.add_noise(inputs_shared["inference_cam2_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device))
        elif training_mode == "cam3":
            # cam3作为条件: cam3分支条件固定，预测 cam1, cam2, cam4
            inputs_shared["latents"][2], sigmas = self.scheduler.add_noise(inputs_shared["inference_cam3_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device))
        elif training_mode == "cam4":
            # cam4作为条件: cam4分支条件固定，预测 cam1, cam2, cam3
            inputs_shared["latents"][3], sigmas = self.scheduler.add_noise(inputs_shared["inference_cam4_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device))
        for progress_id, timestep in enumerate(tqdm(scheduler_copy.timesteps)):
            timestep = timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device)
            # NVS任务的推理timestep设置: [cam1, cam2, cam3, cam4]
            if training_mode == "Generation":
                timestep = [
                    timestep,
                    timestep,
                ...
                # cam1作为条件: cam1分支固定timestep=999，预测 cam2, cam3, cam4
                timestep = [
                    self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                    timestep,
                    timestep,
                ...
                # cam2作为条件: cam2分支固定timestep=999，预测 cam1, cam3, cam4
                timestep = [
                    timestep,
                    self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                    timestep,
                ...
                # cam3作为条件: cam3分支固定timestep=999，预测 cam1, cam2, cam4
                timestep = [
                    timestep,
                    timestep,
                    self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                ...
                # cam4作为条件: cam4分支固定timestep=999，预测 cam1, cam2, cam3
                timestep = [
                    timestep,
                    timestep,
                    timestep,
            ...
            if training_mode == "Generation":
                ...
                # cam3作为条件: 只更新 cam1, cam2, cam4 (索引0,1,3)
                ...
                # cam4作为条件: 只更新 cam1, cam2, cam3 (索引0,1,2)
        ...
        # Decode - NVS任务的解码: [cam1, cam2, cam3, cam4]
        self.load_models_to_device(['vae'])
        video_dict = {}

        if training_mode == "Generation":
            video_cam1 = self.vae.decode(inputs_shared["latents"][[0]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video_cam1 = self.vae_output_to_video(video_cam1, pattern="B C T H W")
            video_dict["cam1"] = video_cam1 # torch.tensor[c,t,h,w]
            video_cam2 = self.vae.decode(inputs_shared["latents"][[1]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video_cam2 = self.vae_output_to_video(video_cam2, pattern="B C T H W")
            video_dict["cam2"] = video_cam2 # torch.tensor[c,t,h,w]
            video_cam3 = self.vae.decode(inputs_shared["latents"][[2]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video_cam3 = self.vae_output_to_video(video_cam3, pattern="B C T H W")
            video_dict["cam3"] = video_cam3 # torch.tensor[c,t,h,w]
            video_cam4 = self.vae.decode(inputs_shared["latents"][[3]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video_cam4 = self.vae_output_to_video(video_cam4, pattern="B C T H W")
            video_dict["cam4"] = video_cam4 # torch.tensor[c,t,h,w]
        elif training_mode == "cam1":
            video_cam2 = self.vae.decode(inputs_shared["latents"][[1]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video_cam2 = self.vae_output_to_video(video_cam2, pattern="B C T H W")
            video_dict["cam2"] = video_cam2 # torch.tensor[c,t,h,w]
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    # NVS任务的输入编码: [cam1, cam2, cam3, cam4]
    def __init__(self):
        super().__init__(
            input_params=("input_videos", "noise", "tiled", "tile_size", "tile_stride", "is_inference", "modality_index", "inference_cam1", "inference_cam2", "inference_cam3", "inference_cam4", "height", "width", "training_mode"),
            onload_model_names=("vae",)
        )
    def process(self, pipe: WanVideoPipeline, input_videos, noise, tiled, tile_size, tile_stride, is_inference, modality_index, inference_cam1, inference_cam2, inference_cam3, inference_cam4, height, width, training_mode):
        if is_inference:
            ...
            if training_mode == "Generation":
                ...
                # cam1作为条件: 需要cam1作为条件
                if inference_cam1 is not None:
                    if isinstance(inference_cam1, Image.Image):
                        inference_cam1 = pipe.preprocess_image(inference_cam1.resize((width, height))).to(pipe.device).to(pipe.torch_dtype)[:,:,None]
                    elif isinstance(inference_cam1, torch.Tensor):
                        inference_cam1 = pipe.preprocess_video(inference_cam1)
                    inference_cam1_latents = pipe.vae.encode(inference_cam1, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
                    result["inference_cam1_latents"] = inference_cam1_latents
                return result
            
            elif training_mode == "cam2":
                # cam2作为条件: 需要cam2作为条件
                if inference_cam2 is not None:
                    if isinstance(inference_cam2, Image.Image):
                        inference_cam2 = pipe.preprocess_image(inference_cam2.resize((width, height))).to(pipe.device).to(pipe.torch_dtype)[:,:,None]
                    elif isinstance(inference_cam2, torch.Tensor):
                        inference_cam2 = pipe.preprocess_video(inference_cam2)
                    inference_cam2_latents = pipe.vae.encode(inference_cam2, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
                    result["inference_cam2_latents"] = inference_cam2_latents
                return result
            
            elif training_mode == "cam3":
                # cam3作为条件: 需要cam3作为条件
                if inference_cam3 is not None:
                    if isinstance(inference_cam3, Image.Image):
                        inference_cam3 = pipe.preprocess_image(inference_cam3.resize((width, height))).to(pipe.device).to(pipe.torch_dtype)[:,:,None]
                    elif isinstance(inference_cam3, torch.Tensor):
                        inference_cam3 = pipe.preprocess_video(inference_cam3)
                    inference_cam3_latents = pipe.vae.encode(inference_cam3, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
                    result["inference_cam3_latents"] = inference_cam3_latents
                return result
            
            elif training_mode == "cam4":
                # cam4作为条件: 需要cam4作为条件
                if inference_cam4 is not None:
                    if isinstance(inference_cam4, Image.Image):
                        inference_cam4 = pipe.preprocess_image(inference_cam4.resize((width, height))).to(pipe.device).to(pipe.torch_dtype)[:,:,None]
                    elif isinstance(inference_cam4, torch.Tensor):
                        inference_cam4 = pipe.preprocess_video(inference_cam4)
                    inference_cam4_latents = pipe.vae.encode(inference_cam4, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
                    result["inference_cam4_latents"] = inference_cam4_latents
                return result
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # Ensure each timestep[i] is 1-D to avoid torch.outer receiving 0-D scalars
    t = torch.cat([
        dit.time_embedding(
            sinusoidal_embedding_1d(dit.freq_dim, timestep[i].reshape(-1))
...
class WanTrainingModule_wan2_1_14b_t2v_nvs_lora_train_v1(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        self.pipe = WanVideoPipeline.from_pretrained(torch_dtype=torch.bfloat16, device="cpu", tokenizer_config=tokenizer_config, model_configs=model_configs,redirect_common_files=False) # 加载预训练权重
    ...
    def training_mode(self) -> str:
        ...
        if dist.is_available() and dist.is_initialized():
            ...
            if dist.get_rank() == 0:
                ...
                mode_idx = 0  # 其他rank先初始化
            
            # 广播到所有rank
            mode_tensor = torch.tensor([mode_idx], dtype=torch.long, device='cuda')
            dist.broadcast(mode_tensor, src=0)
            mode_idx = mode_tensor.item()
            
            # 映射回字符串
            modes = ["Generation", "cam1", "cam2", "cam3", "cam4"]
    ...
    def forward_preprocess(self, data):
        ...
        # 所有NVS任务都使用prompt
        if inputs_shared["training_mode"] == "Generation":
            inputs_posi = {"prompt": data["prompt"]}
        else:
            inputs_posi = {"prompt": ""}
        input_videos = torch.cat((data["cam1"], data["cam2"], data["cam3"], data["cam4"]), dim=0) # 4b c t h w
