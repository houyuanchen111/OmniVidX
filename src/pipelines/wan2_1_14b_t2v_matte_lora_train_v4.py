"""
针对Generation的时候会闪烁的问题，提出一个可能的解决方案：不同的branch接受不同的prompt
""" 
import torchvision
...
class WanTrainingModule_wan2_1_14b_t2v_matte_lora_train_v4(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        if dist.is_available() and dist.is_initialized():
            ...
            if dist.get_rank() == 0:
                ...
                if flag < 1/3:
                    ...
                    mode_idx = 2  # "BackgroundGeneration"
            else:
            ...
            # 映射回字符串
            modes = ["RGBAGeneration", "Matte", "BackgroundGeneration"]
    ...
    def forward_preprocess(self, data):
        ...
        if inputs_shared["training_mode"] == "Matte":
            prompt_list = [""]
            prompt_list.append(data["fgr_prompt"][0] + "白色剪影，纯黑色背景。")
            prompt_list.append(data["fgr_prompt"][0] + "纯黑色背景。")
            prompt_list.append(data["bg_prompt"][0])
            inputs_posi = {"prompt": prompt_list}
        elif inputs_shared["training_mode"] == "RGBAGeneration":
            prompt_list = data["prompt"].copy()
            prompt_list.append(data["fgr_prompt"][0] + "白色剪影，纯黑色背景。")
            prompt_list.append(data["fgr_prompt"][0] + "纯黑色背景。")
            prompt_list.append(data["bg_prompt"][0])
            inputs_posi = {"prompt": prompt_list}
