...
class WanVideoPipeline(BasePipeline):
    ...
    def fork(self, clone_dit: bool = False, separate_scheduler: bool = True):
        """
        轻量克隆当前 pipeline：
        - 默认共享大模块权重（text/image encoder、vae、motion_controller、vace、prompter）
        - 可选 deep copy dit（当各分支 LoRA 不同且需互不影响时，设置 clone_dit=True）
        - 可选独立 scheduler（分支各自维护采样/训练步）
        - units 与 unit_runner 使用新的实例，请按分支需求在外部设置
        """
        newp = self.__class__(device=self.device, torch_dtype=self.torch_dtype,
                              tokenizer_path=getattr(self.prompter, "tokenizer_path", None))
        # 共享的大模块，避免显存翻倍
        newp.text_encoder = self.text_encoder
        newp.image_encoder = self.image_encoder
        newp.vae = self.vae
        newp.motion_controller = self.motion_controller
        newp.vace = self.vace
        newp.prompter = self.prompter
        # dit：根据需要克隆或共享
        newp.dit = copy.deepcopy(self.dit) if clone_dit else self.dit
        # 运行相关
        newp.model_fn = self.model_fn
        newp.in_iteration_models = self.in_iteration_models
        newp.unit_runner = PipelineUnitRunner()
        newp.units = None
        newp.scheduler = copy.deepcopy(self.scheduler) if separate_scheduler else self.scheduler
        return newp
    ...
    def __call__(
        ...
        **kwargs 
        ...
        modality_data = {
                "modality_index": None,
                "input_videos": [data.get("material", None)],
                "material": [data.get("material", None)],
            }
        # if modality_index == 0:
        #     modality_data = {
        #         "modality_index": 0,
        #         "input_videos": [data.get("albedo", None)],
        #         "albedo": [data.get("albedo", None)], 
        #     }
        # elif modality_index == 1:
        #     modality_data = {
        #         "modality_index": 1,
        #         "input_videos": [data.get("basecolor", None)],
        #         "basecolor": [data.get("basecolor", None)],
        #     }
        # elif modality_index == 2:
        #     modality_data = {
        #         "modality_index": 2,
        #         "input_videos": [data.get("depth", None)],
        #     }
        ...
        # else:
        #     raise ValueError(f"modality_index must be in [0, 1, 2, 3, 4], but got {modality_index}")
        inputs_shared.update(modality_data)
        ...
        video_dict = {}
        inputs_shared["modality_index"] = torch.tensor([3], device=self.device) # 强制算normal的loss
        ...
        if video_dict["modality_index"].item() == 0:
            ...
            #  按T维度循环归一化到[-1,1]范围
            #video_albedo = self.normalize_tensor_by_time_to_neg1_pos1(video_albedo)
            ...
            #video_basecolor_unit = self.normalize_tensor_by_time_to_neg1_pos1(video_basecolor_unit)
            ...
            video_depth_unit = self.normalize_tensor_by_time_to_neg1_pos1(video_depth_unit) # depth是需要滴
            video_dict["depth"] = video_depth_unit # torch.tensor[c,t,h,w]
        elif video_dict["modality_index"].item() == 3:
            # c == 3
            video_material = self.vae.decode(inputs_shared["latents"], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            ...
            #roughness = self.normalize_tensor_by_time_to_neg1_pos1(roughness)
            #metallic = self.normalize_tensor_by_time_to_neg1_pos1(metallic)
...
class WanTrainingModule_wan2_1_fun_1_3b_control_multi_task_lora_image_train_v0(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        #self.pipe.dit.context_embedding = torch.nn.Embedding(16, 1536).to(device).to(torch.bfloat16) # follow diffusion renderer # 不需要embed
        ...
        # 全部冻结
        # for param in self.pipe.dit.context_embedding.parameters():
        ...
        trainable_params = sum(p.numel() for p in self.pipe.parameters() if p.requires_grad)
        total_params = sum(p.numel() for p in self.pipe.parameters())
        print(f"Trainable after LoRA: {trainable_params/1024/1024:.2f} M / {total_params/1024/1024:.2f} M") # 只有16M
        # Store other configs
        self.use_gradient_checkpointing = use_gradient_checkpointing
        self.use_gradient_checkpointing_offload = use_gradient_checkpointing_offload
        self.extra_inputs = extra_inputs.split(",") if extra_inputs is not None else []
        
        # 可选：创建多分支，每个分支独立 dit 并加载不同 LoRA
        self.pipes = {}
        # 示例：当需要两个分支时，按需启用并替换路径
        # branch_names_and_lora = {
        #     "A": "/abs/path/to/lora_A.safetensors",
        #     "B": "/abs/path/to/lora_B.safetensors",
        # }
        # for name, lora_path in branch_names_and_lora.items():
        #     p = self.pipe.fork(clone_dit=True, separate_scheduler=True)
        #     # 分支 units 可按需不同
        #     p.units = [
        #         WanVideoUnit_ShapeChecker(),
        #         WanVideoUnit_Modality_Embedder(),
        ...
        #     ]
        #     # 重置调度器（训练/推理各自维护）
        #     p.scheduler.set_timesteps(1000, training=True)
        #     # 仅对分支的 dit 挂载不同 LoRA
        #     self.add_lora_to_model(p.dit, target_modules=lora_target_modules.split(","), lora_rank=lora_rank)
        #     # 如需从文件加载 LoRA 权重
        #     # p.load_lora(p.dit, lora_path, alpha=1)
        #     self.pipes[name] = p
    ...
    def forward_preprocess(self, data):
        ...
        # mixed modality data
        inputs_shared.update(self.get_modality_data(data,"material"))
        inputs_shared["modality_index"] = None # 
