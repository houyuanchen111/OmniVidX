...
from ..models.wan_video_dit_matte_v9_w_channel_concat import WanModel, RMSNorm, sinusoidal_embedding_1d
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    context = dit.text_embedding(context)[[0]] # mlp -> silu -> mlp # b l c； c 的维度要和dit的channel对上，L没关系, 只取一个就行

    x = latents # noise
    x = rearrange(x, "b c t h w -> 1 (b c) t h w") # 变成channel concat
    B = x.shape[0]
    # Merged cfg
    ...
    x = dit.patch_out(x) # type: ignore
    # x = rearrange(x, "1 (b c) t h w -> b c t h w", b=4)
    x = rearrange(x[[0]], "1 (b c) t h w -> b c t h w", b=4)
    return x

class WanTrainingModule_wan2_1_14b_t2v_matte_lora_train_v9_w_channel_concat(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        # 放开 WanModel 中的 patch_embedding_channel_concat 参数
        if hasattr(self.pipe.dit, 'patch_embedding_channel_concat'):
            module_params = 0
            for param in self.pipe.dit.patch_embedding_channel_concat.parameters():
                param.requires_grad = True
                module_params += param.numel()
                trainable_params += param.numel()
            trainable_modules.append(("patch_embedding_channel_concat", module_params))
            print(f"Trainable: patch_embedding_channel_concat ({module_params /1024/1024:.2f} MB)")
        # 放开 WanModel 中的 patch_out 参数
        if hasattr(self.pipe.dit, 'patch_out'):
            module_params = 0
            for param in self.pipe.dit.patch_out.parameters():
                param.requires_grad = True
                module_params += param.numel()
                trainable_params += param.numel()
            trainable_modules.append(("patch_out", module_params))
            print(f"Trainable: patch_out ({module_params /1024/1024:.2f} MB)")
    ...
    def forward_preprocess(self, data):
        ...
        rgb_output_modes = ["t2RPFB", "P2RFB", "F2RPB", "B2RPF", "PF2RB", "PB2RF", "FB2RP"]
