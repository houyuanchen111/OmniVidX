...
from ..models.wan_video_dit_pbr_sft_rec_gen_rm_rgblora import WanModel, RMSNorm, sinusoidal_embedding_1d
...
from ..renderer.forward_render import run_rendering
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        inputs["input_latents"] = inputs["input_latents"].to(dtype=self.torch_dtype, device=self.device)
        ...
        ### render loss
        # print(next(self.vae.model.decoder.parameters()).device)
        # if next(self.vae.model.decoder.parameters()).device != "cpu":
        #     self.vae.model.decoder.to("cpu")
        #     self.vae.model.conv2.to("cpu")
        #     print(next(self.vae.model.decoder.parameters()).device)
        print("begin render loss")
        render_mode = random.randint(0, 1)
        sigma = sigmas[1].item()
        z0_predict_frame_0 = inputs["latents"][1:][:,:,[0]] - sigma * noise_pred[1:][:,:,[0]] # 只对pbr的模态进行render loss的监督,而且只取第一帧
        cuda_device = z0_predict_frame_0.device
        # os.environ.pop('TQDM_DISABLE', None)
        albedo_gt = inputs["input_videos"][0][1] * 0.5 + 0.5
        material_gt = inputs["input_videos"][0][2] * 0.5 + 0.5
        normal_gt = inputs["input_videos"][0][3] # normal就是-1 -1之间就行
        loss_render = 0
        weight_init = np.random.uniform(0, 3)
        longtitude = np.random.uniform(0, np.pi)
        latitude = np.random.uniform(-np.pi / 2, np.pi / 2)
        with torch.no_grad():
            render_gt = run_rendering(
                albedo_gt[:, 0].float().to(cuda_device), 
                material_gt[0, 0].repeat(3,1,1).float().to(cuda_device), 
                material_gt[1, 0].repeat(3,1,1).float().to(cuda_device), 
                normal_gt[:, 0].float().to(cuda_device),
                lighting_longitude = torch.tensor(longtitude).to(cuda_device).float().requires_grad_(True), 
                lighting_latitude = torch.tensor(latitude).to(cuda_device).float().requires_grad_(True), 
                weight_init=weight_init
            )
        if render_mode == 0:
            # 只使用normal_predict
            normal_predict = torch.utils.checkpoint.checkpoint(
                create_custom_forward(self.vae.decode),
                z0_predict_frame_0[[2]],
                cuda_device,
                inputs["tiled"],
                inputs["tile_size"],
                inputs["tile_stride"],
            ...
            render_predict = run_rendering(
                albedo_gt[:,0].float().to(cuda_device), 
                material_gt[0,0].repeat(3,1,1).float().to(cuda_device), 
                material_gt[1,0].repeat(3,1,1).float().to(cuda_device), 
                normal_predict[:,0].float().to(cuda_device), # normal就是-1 -1之间就行
                lighting_longitude = torch.tensor(longtitude).to(cuda_device).float().requires_grad_(True), 
                lighting_latitude = torch.tensor(latitude).to(cuda_device).float().requires_grad_(True), 
                weight_init=weight_init
            )
        elif render_mode == 1:
            # 只使用material_predict[1]
            material_predict = torch.utils.checkpoint.checkpoint(
                create_custom_forward(self.vae.decode),
                z0_predict_frame_0[[1]],
                cuda_device,
                inputs["tiled"],
                inputs["tile_size"],
                inputs["tile_stride"],
            ...
            render_predict = run_rendering(
                albedo_gt[:,0].float().to(cuda_device), 
                material_gt[0,0].repeat(3,1,1).float().to(cuda_device), 
                material_predict[1].repeat(3,1,1).float().to(cuda_device) * 0.5 + 0.5, 
                normal_gt[:,0].float().to(cuda_device),
                lighting_longitude = torch.tensor(longtitude).to(cuda_device).float().requires_grad_(True), 
                lighting_latitude = torch.tensor(latitude).to(cuda_device).float().requires_grad_(True), 
                weight_init=weight_init
            ...
            render_predict = run_rendering(
                albedo_gt[:,0].float().to(cuda_device), 
                material_predict[0].repeat(3,1,1).float().to(cuda_device) * 0.5 + 0.5, 
                material_gt[1,0].repeat(3,1,1).float().to(cuda_device), 
                normal_gt[:,0].float().to(cuda_device),
                lighting_longitude = torch.tensor(longtitude).to(cuda_device).float().requires_grad_(True), 
                lighting_latitude = torch.tensor(latitude).to(cuda_device).float().requires_grad_(True), 
            ...
            # 只使用albedo_predict
            albedo_predict = torch.utils.checkpoint.checkpoint(
                create_custom_forward(self.vae.decode),
                z0_predict_frame_0[[0]],
                cuda_device,
                inputs["tiled"],
                inputs["tile_size"],
                inputs["tile_stride"],
            ...
            render_predict = run_rendering(
                albedo_predict[:,0].float().to(cuda_device) * 0.5 + 0.5, 
                material_gt[0,0].repeat(3,1,1).float().to(cuda_device), 
                material_gt[1,0].repeat(3,1,1).float().to(cuda_device), 
                normal_gt[:,0].float().to(cuda_device),
                lighting_longitude = torch.tensor(longtitude).to(cuda_device).float().requires_grad_(True), 
        ...
        loss_render += torch.nn.functional.mse_loss(render_predict.float(), render_gt.float()) # 每次都只监督一种模态行不行呢？# loss render确实就不该太大，因为要先训差不多了再用这个loss

        print(f"loss_render: {loss_render}, loss: {loss}")
        loss = (loss + loss_render) * self.scheduler.training_weight(timestep_weight)
        return loss
    ...
    def from_pretrained(
        ...
        # Load models
        # pipe.text_encoder = model_manager.fetch_model("wan_video_text_encoder") # type: ignore # 不需要加这个了
    ...
    def __call__(
        ...
        **kwargs 
        ...
        video_dict = {}
        self.vae.to(self.device) # 确保这边vae是在gpu上面的
        if is_generation:
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            seperate_cfg=True, # 
            input_params_posi={"prompt": "prompt", "positive": "positive", "context": "context"},
            input_params_nega={"prompt": "negative_prompt", "positive": "positive", "context": "context"},
            onload_model_names=("text_encoder",)
        )

    def process(self, pipe: WanVideoPipeline, prompt, positive, context) -> dict:
        if context is not None:
            prompt_emb = context.to(pipe.device).to(pipe.torch_dtype)
        else:
            prompt_emb = torch.load("blank.pth")["embedding"].to(pipe.device).to(pipe.torch_dtype)
...
class WanTrainingModule_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_rm_rgblora_render_loss_v1_lora_video_train_v4(DiffusionTrainingModule): 
    def __init__(
        self,
        model_paths=None, model_id_with_origin_paths=None,
        trainable_models=None,
        lora_base_model=None, lora_target_modules="q,k,v,o,ffn.0,ffn.2", lora_rank=32, lora_modalities:list[str]=None,
        use_gradient_checkpointing=True, # 改成True 
        ...
        self.pipe.vae.to('cpu') # vae放到cpu上以节省显存
    ...
    def forward_preprocess(self, data):
        ...
        if not inputs_shared["is_generation"]:
            inputs_posi = {"prompt": "", "context": None} # 重建的时候，promot置空
        else:
            inputs_posi = {"prompt": data["prompt"], "context": data["context"]}
        # 只试试albedo的效果
