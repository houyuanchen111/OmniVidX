...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        albedo_predict = self.vae.decode(z0_predict[[0]], device=self.device, tiled=inputs["tiled"], tile_size=inputs["tile_size"], tile_stride=inputs["tile_stride"])[0]
        material_predict = self.vae.decode(z0_predict[[1]], device=self.device, tiled=inputs["tiled"], tile_size=inputs["tile_size"], tile_stride=inputs["tile_stride"])[0]
        ...
        albedo_gt = inputs["input_videos"][0][1]
        material_gt = inputs["input_videos"][0][2]
        normal_gt = inputs["input_videos"][0][3]
        loss_render_albedo = torch.nn.functional.mse_loss(albedo_predict.float().to(albedo_gt.device), albedo_gt.float())
        loss_render_material = torch.nn.functional.mse_loss(material_predict.float().to(material_gt.device), material_gt.float()) / 2 
        loss_render_normal = torch.nn.functional.mse_loss(normal_predict.float().to(normal_gt.device), normal_gt.float())
        
        loss_rec = (loss_render_albedo + loss_render_material + loss_render_normal) / 3
        #### render loss
        loss_rec_weight = -sigma * sigma + 1 # 可能要消融一下这个weight
        # print((loss_render_albedo + loss_render_material + loss_render_normal) / 3)
        # print(loss)
        loss = loss * self.scheduler.training_weight(timestep_weight) + loss_rec_weight * loss_rec
        return loss
...
class WanTrainingModule_wan2_1_1_3b_t2v_pbrgen_pbr_sft_rec_gen_rec_loss_v0_lora_image_train_v1(DiffusionTrainingModule): 
    def __init__(
        self,
