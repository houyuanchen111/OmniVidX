"""
看能不能用lora训出一个t2 albedo video 的model
""" 
...
from ..models.wan_video_dit_pbr_alignment import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device):
        ...
        noise = pipe.generate_noise((3, 16, length, height//8, width//8), seed=seed, rand_device=rand_device) # 三种模态的noise应该不一样
        return {"noise": noise}
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, prompt, positive) -> dict:
        ...
        prompt_emb = prompt_emb.repeat(3, 1, 1) # 三种模态的prompt处理成一样的就行了
...
class WanTrainingModule_wan2_1_1_3b_t2v_pbrgen_pbr_alignment_lora_video_train_v0(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        if lora_base_model is not None:
            ...
            for name, param in self.pipe.dit.named_parameters():
                if "lora" in name.lower() and "normal" not in name.lower(): # 不放开normal的LoRA
    ...
    def forward_preprocess(self, data):
        ...
        # 只试试albedo的效果
        input_videos = torch.cat((data["albedo"], data["material"], data["normal"]), dim=0) # 4b c t h w
