...
class WanTrainingModule_wan2_1_14b_t2v_pbr_lora_video_train_v8(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        rgb_resume_from_checkpoint: Optional[str] = None,
        ...
        if rgb_resume_from_checkpoint is not None:
            state_dict = load_file(rgb_resume_from_checkpoint) # safetensors 不需要map_location
            prefix = "dit." # 去掉不想要的前缀
            state_dict = {k[len(prefix):] if k.startswith(prefix) else k: v for k, v in state_dict.items()}
            state_dict = {k.replace("default", "rgb"): v for k, v in state_dict.items()} # 重命名
            try:
                missing_keys, unexpected_keys = self.pipe.dit.load_state_dict(state_dict, strict=False)
                assert unexpected_keys == [], "unexpected_keys not empty"
            except Exception as e:
                print(f"从checkpoint {rgb_resume_from_checkpoint} 恢复训练失败: {e}")
            if missing_keys:
                print(f"⚠️ 缺失键: {len(missing_keys)} 个")
                # 显示前几个缺失键作为示例
                for key in list(missing_keys)[:5]:
