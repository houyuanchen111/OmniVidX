"""
ablation 不用lora，改成fft，并且用image
""" 
import torchvision
...
from ..models.wan_video_dit_pbr_sft_rec_gen_rm_rgblora_sft import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        ### render loss
        render_mode = random.randint(0, 3)
        sigma = sigmas[1].item()
...
class WanTrainingModule_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_rm_rgblora_render_loss_v1_lora_image_ablation_fft_train_v2(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        if hasattr(self.pipe.dit, 'domain_embedding'):
            print(f"Trainable: domain_embedding")
            module_params = 0
        ...
        for name, module in self.pipe.dit.named_modules():
            if hasattr(module, 'patch_align_modulation'):
                print(f"Trainable: {name}.patch_align_modulation")
        ...
        if hasattr(self.pipe.dit, 'res_patch_embedding'):
            print(f"Trainable: res_patch_embedding")
        ...
        if hasattr(self.pipe.dit, 'res_patch_embedding_gate'):
            print(f"Trainable: res_patch_embedding_gate")
        ...
        print(f"\n放开dit的所有权重:")
        for name, param in self.pipe.dit.named_parameters():
            if not param.requires_grad:  # 只处理之前被冻结的参数
                param.requires_grad = True
                param_count = param.numel()
                trainable_params += param_count
                print(f"Trainable: {name} ({param_count /1024/1024:.2f} MB)")
