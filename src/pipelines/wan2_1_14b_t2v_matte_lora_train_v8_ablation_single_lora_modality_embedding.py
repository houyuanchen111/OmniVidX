...
from ..models.wan_video_dit_matte_v3 import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        elif training_mode == "ForegroundGeneration":
            timestep = [
                timestep_weight,
                timestep_weight,
                timestep_weight,
        ...
        if training_mode == "RGBAGeneration":
            ...
            # 只计算 fgr 分支的loss (索引2)，根据 pha 抠出前景
            loss = torch.nn.functional.mse_loss(noise_pred.float()[[2]], training_target.float()[[2]])
        elif training_mode == "ForegroundGeneration":
            # 只计算 com, pha, fgr 三个分支的loss (索引0,1,2)，根据 bgr 生成前景相关分支
        ...
        **kwargs 
        ...
        for progress_id, timestep in enumerate(tqdm(scheduler_copy.timesteps)):
            ...
            if training_mode == "RGBAGeneration":
                ...
                # Masking 任务: com, pha, bgr 分支固定timestep=999，仅预测 fgr 分支
                timestep = [
                    self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                    self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                    timestep,
        ...
        if training_mode == "RGBAGeneration":
            ...
            # Masking 任务: 主要关心 fgr 分支
            video_fgr = self.vae.decode(inputs_shared["latents"][ [2] ], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video_fgr = self.vae_output_to_video(video_fgr, pattern="B C T H W")
            video_dict["fgr"] = video_fgr # torch.tensor[c,t,h,w]
        elif training_mode == "ForegroundGeneration":
            # 前景生成: 输出 com, pha, fgr 三个分支
            video_com = self.vae.decode(inputs_shared["latents"][ [0] ], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video_com = self.vae_output_to_video(video_com, pattern="B C T H W")
            video_dict["com"] = video_com # torch.tensor[c,t,h,w]
            video_pha = self.vae.decode(inputs_shared["latents"][ [1] ], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
    ...
    def process(self, pipe: WanVideoPipeline, input_videos, noise, tiled, tile_size, tile_stride, is_inference, modality_index, inference_com, inference_pha, inference_fgr, inference_bgr, height, width, training_mode):
        if is_inference:
            ...
            if training_mode == "RGBAGeneration":
                ...
                # Masking 任务: 主要根据 com/pha/bgr 进行 fgr 抠取
                if inference_com is not None:
                    if isinstance(inference_com, Image.Image):
                        inference_com = pipe.preprocess_image(inference_com.resize((width, height))).to(pipe.device).to(pipe.torch_dtype)[:,:,None]
                    elif isinstance(inference_com, torch.Tensor):
                ...
                # 前景生成: 使用背景 bgr 作为条件，生成 com/pha/fgr
                if inference_bgr is not None:
                    if isinstance(inference_bgr, Image.Image):
                        inference_bgr = pipe.preprocess_image(inference_bgr.resize((width, height))).to(pipe.device).to(pipe.torch_dtype)[:,:,None]
                    elif isinstance(inference_bgr, torch.Tensor):
...
class WanTrainingModule_wan2_1_14b_t2v_matte_lora_train_v8_ablation_single_lora_modality_embedding(DiffusionTrainingModule): 
    def __init__(
        self,
    ...
    def training_mode(self) -> str:
        ...
        modes = ["RGBAGeneration", "Matte", "BackgroundGeneration", "Masking", "ForegroundGeneration"]
    ...
    def forward_preprocess(self, data):
        ...
        if inputs_shared["training_mode"] == "Matte":
            ...
            inputs_posi = {"prompt": prompt_list}
        elif inputs_shared["training_mode"] == "Masking":
            prompt_list = [
                "",
                "",
                "",
            ...
            inputs_posi = {"prompt": prompt_list}
        elif inputs_shared["training_mode"] == "ForegroundGeneration":
            prompt_list = [
                data["prompt"][0],
                data["prompt"][0],
                data["prompt"][0],
