"""
尝试使用gradient checkpoint技术， 看能否降低preprocess地方的显存占用？
""" 
import torchvision
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_videos, noise, tiled, tile_size, tile_stride, is_inference, modality_index, inference_image, height, width):
        ...
        input_latents = torch.utils.checkpoint.checkpoint(
            create_custom_forward(pipe.vae.encode),
            video,
            pipe.device,
            tiled,
            tile_size,
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, prompt, positive) -> dict:
        ...
        prompt_emb = torch.utils.checkpoint.checkpoint(
            create_custom_forward(pipe.prompter.encode_prompt),
            prompt,
            positive,
            str(pipe.device),
        ) # 如果device本身就已经是str了，str()会返回原值
...
class WanTrainingModule_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_rm_rgblora_render_loss_v1_lora_video_train_v3(DiffusionTrainingModule): 
    def __init__(
        self,
    ...
    def forward_preprocess(self, data):
        ...
        if not inputs_shared["is_generation"]:
            inputs_posi = {"prompt": ""} # 重建的时候，promot置空
        else:
