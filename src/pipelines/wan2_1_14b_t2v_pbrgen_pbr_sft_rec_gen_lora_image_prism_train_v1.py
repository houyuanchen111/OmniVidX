...
from ..models.wan_video_dit_pbr_sft_rec_gen_prism import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        if training_mode == "RGBX_Generation":
            timestep = [
                timestep_weight,
                timestep_weight,
                timestep_weight,
        ...
        elif training_mode == "Normal2RGBX":
            timestep = [
                timestep_weight,
                timestep_weight,
                timestep_weight,
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                ] # normal branch timestep为999 近似于不加噪
        elif training_mode == "Relight":
            timestep = [
                timestep_weight,
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                timestep_weight,
    ...
    def __call__(
        ...
        **kwargs 
        ...
        if training_mode == "RGB2X":
            ...
            inputs_shared["latents"][3], sigmas = self.scheduler.add_noise(inputs_shared["inference_normal_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device)) # 替换成normal 
        elif training_mode == "Relight":
        ...
        if training_mode not in ["Relight", "Normal2RGBX"]:
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def __init__(self):
        super().__init__(
            input_params=("input_videos", "noise", "tiled", "tile_size", "tile_stride", "is_inference", "modality_index", "inference_rgb", "inference_albedo", "inference_normal", "height", "width", "training_mode"),
            onload_model_names=("vae",)
        )
    def process(self, pipe: WanVideoPipeline, input_videos, noise, tiled, tile_size, tile_stride, is_inference, modality_index, inference_rgb, inference_albedo, inference_normal, height, width, training_mode):
        if is_inference:
            if training_mode == "RGBX_Generation":
...
class WanTrainingModule_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_lora_image_prism_train_v1(DiffusionTrainingModule): 
    def __init__(
        self,
    ...
    def forward_preprocess(self, data):
        ...
        if inputs_shared["training_mode"] == "RGBX_Generation":
            inputs_posi = {"prompt": ""} # 重建的时候，promot置空
        else:
            inputs_posi = {"prompt": data["prompt"]}
        # 只试试albedo的效果
        input_videos = torch.cat((data["im"], data["albedo"], data["irradiance"], data["normal"]), dim=0) # 4b c t h w
