...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        **kwargs 
        ...
        video_dict = {}
        inputs_shared["modality_index"] = torch.tensor([0], device=self.device) # 强制算normal的loss
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if dit.has_image_input: # y 是 ctrl video latent 
        ...
        clip_embdding = dit.img_emb(clip_feature) # L相当大,  为什么？
...
class WanTrainingModule_wan2_1_fun_1_3b_control_albedo_lora_image_train_v0(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        if lora_ckpt_path is not None:
            try:
                self.pipe.load_lora(self.pipe.dit, lora_ckpt_path)
            except Exception as e:
                print(f"从checkpoint {lora_ckpt_path} 加载LoRA失败: {e}")
    ...
    def forward_preprocess(self, data):
        ...
        # mixed modality data
        inputs_shared.update(self.get_modality_data(data,"albedo"))
