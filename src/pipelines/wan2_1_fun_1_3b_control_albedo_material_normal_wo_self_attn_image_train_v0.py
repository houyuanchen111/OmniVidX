"""
新实验，cross modality的能力仅仅放在新加的patch align attention上面，以减少显存消耗
""" 
...
from ..models.wan_video_dit_albedo_material_normal_wo_self_attn_v0 import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        timestep_id = torch.randint(0, self.scheduler.num_train_timesteps, (1,)) # 一共1000个timestep，随机取一个整数
    ...
    def from_pretrained(
        ...
        # pipe.dit = model_manager.fetch_model("wan_video_dit") # type: 
        # 用这种方式加载，之前那个太复杂，不方便加模块
        # 从 JSON 读取 WanModel 配置并实例化，然后加载 safetensors 权重
        config_path = "configs/wan2_1_fun_1_3b_control_dit_config.json"
        ckpt_path = "checkpoints/Wan2.1-Fun-1.3B-Control/diffusion_pytorch_model.safetensors"
    ...
    def __call__(
        ...
        **kwargs 
        ...
        b, c, t, h, w = data["rgb"].shape
        inputs_shared["batch_size"] = b
        input_videos = torch.cat((data["albedo"], data["material"], data["normal"]), dim=0) # 3b c t h w, albedo, material, normal
        ...
        video_albedo = self.vae.decode(inputs_shared["latents"][[0]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_material = self.vae.decode(inputs_shared["latents"][[1]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_normal = self.vae.decode(inputs_shared["latents"][[2]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("height", "width", "num_frames", "seed", "rand_device", "batch_size"))

    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, batch_size):
        length = (num_frames - 1) // 4 + 1 
        noise_pair = pipe.generate_noise((3, 16, length, height//8, width//8), seed=seed, rand_device=rand_device)
        rep = (batch_size,) + (1,) * (noise_pair.ndim - 1)
        noise = torch.cat([
            noise_pair[0:1].repeat(*rep),
            noise_pair[1:2].repeat(*rep),
            noise_pair[2:3].repeat(*rep),
        ], dim=0)
        return {"noise": noise}
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, prompt, positive) -> dict:
        ...
        if isinstance(prompt, str):
            prompt = [prompt] * 3 # for negative prompt
        elif isinstance(prompt, list):
            # 对列表中的每个prompt先乘以3，再到下一个prompt
            new_prompt = []
            for p in prompt:
                new_prompt.extend([p] * 3)
            prompt = new_prompt
        else:
            raise ValueError("prompt is not a list or string")
        pipe.load_models_to_device(self.onload_model_names)
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    t = dit.time_embedding(sinusoidal_embedding_1d(dit.freq_dim, timestep)) # 1 1536(d)
    ...
    # 为不同模态（albedo，material，normal）添加可学习的 domain embedding
    # x: (B, L, C)
    B = x.shape[0]
    if B % 3 == 0 and hasattr(dit, "domain_embedding"):
        domain_ids = torch.zeros(B, dtype=torch.long, device=x.device)
        domain_ids[B // 3:B // 2] = 1        # 第二个1/3批次设为1（material）
        domain_ids[B // 2:B // 3 * 2] = 2    # 第三个1/3批次设为2（normal）
        # 前1/3批次保持为0（albedo）
...
class WanTrainingModule_wan2_1_fun_1_3b_control_albedo_material_normal_wo_self_attn_image_train_v0(DiffusionTrainingModule): 
    def __init__(
        self,
    ...
    def forward_preprocess(self, data):
        ...
        b, c, t, h, w = data["rgb"].shape
        base_prompts = ["Generate a video showing realistic surface coloring and appearance of objects",
                    "Generate a video displaying detailed material properties and textures like roughness, metallic shine",
                    "Generate a video revealing surface geometry and directional information through shading patterns"]
        
        prompt = [p for p in base_prompts for _ in range(b)]  # 每个prompt重复b次atterns"] 
        ...
        # 在batch维度cat
        input_videos = torch.cat((data["albedo"], data["material"], data["normal"]), dim=0) # 3b c t h w
        inputs_shared["input_videos"] = [input_videos]
        for extra_input in self.extra_inputs:
            if extra_input == "input_image":
                ...
                inputs_shared[extra_input] = torch.cat((data["rgb"], data["rgb"], data["rgb"]), dim=0) # 3b c t h w
            else:
