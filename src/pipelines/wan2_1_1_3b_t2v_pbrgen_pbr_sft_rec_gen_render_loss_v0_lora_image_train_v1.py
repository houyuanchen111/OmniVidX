"""
render loss v0: 仅仅只加render loss，结果会怎样？
""" 
import torchvision
from numpy import vdot
import torch, types, copy,json,random,os,numpy as np
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        #### render loss
        sigma = sigmas[1].item()
        z0_predict = inputs["latents"][1:] - sigma * noise_pred[1:] # 只对pbr的模态进行render loss的监督
        # os.environ['TQDM_DISABLE'] = '1' # 暂时关闭tqdm
        albedo_predict = self.vae.decode(z0_predict[[0]], device=self.device, tiled=inputs["tiled"], tile_size=inputs["tile_size"], tile_stride=inputs["tile_stride"])[0] * 0.5 + 0.5
        material_predict = self.vae.decode(z0_predict[[1]], device=self.device, tiled=inputs["tiled"], tile_size=inputs["tile_size"], tile_stride=inputs["tile_stride"])[0] * 0.5 + 0.5
        normal_predict = self.vae.decode(z0_predict[[2]], device=self.device, tiled=inputs["tiled"], tile_size=inputs["tile_size"], tile_stride=inputs["tile_stride"])[0]
        # os.environ.pop('TQDM_DISABLE', None)
        albedo_gt = inputs["input_videos"][0][1] * 0.5 + 0.5
        material_gt = inputs["input_videos"][0][2] * 0.5 + 0.5
        normal_gt = inputs["input_videos"][0][3]
        loss_render = 0
        for i in range(4):
            # render 五次算loss,这个是最直接的算法
            weight_init = np.random.uniform(0, 3)
            longtitude = np.random.uniform(0, np.pi)
            latitude = np.random.uniform(-np.pi, np.pi)
            render_predict = run_rendering(
                albedo_predict[:,0].float(), 
                material_predict[0].repeat(3,1,1).float(), 
                material_predict[1].repeat(3,1,1).float(), 
                normal_predict[:,0].float(),
                lighting_longitude = torch.tensor(longtitude).to(albedo_predict.device).float().requires_grad_(True), 
                lighting_latitude = torch.tensor(latitude).to(albedo_predict.device).float().requires_grad_(True), 
                weight_init=weight_init
            )
            with torch.no_grad():
                render_gt = run_rendering(
                    albedo_gt[:,0].float().to(albedo_predict.device), 
                    material_gt[0].repeat(3,1,1).float().to(albedo_predict.device), 
                    material_gt[1].repeat(3,1,1).float().to(albedo_predict.device), 
                    normal_gt[:,0].float().to(albedo_predict.device),
                    lighting_longitude = torch.tensor(longtitude).to(albedo_predict.device).float().requires_grad_(True), 
                    lighting_latitude = torch.tensor(latitude).to(albedo_predict.device).float().requires_grad_(True), 
                    weight_init=weight_init
                )
            loss_render += torch.nn.functional.mse_loss(render_predict.float(), render_gt.float())
        # print(loss_render)
        #### render loss
        # if sigma > 0.7 and sigma < 0.9:
        #     render_1 = run_rendering(
        #         albedo_predict[:,0].float(), 
        #         material_predict[0].repeat(3,1,1).float(), 
        #         material_predict[1].repeat(3,1,1).float(), 
        #         normal_predict[:,0].float(),
        #         torch.tensor(0).to(albedo_predict.device).float().requires_grad_(True), 
        #         torch.tensor(np.pi/2).to(albedo_predict.device).float().requires_grad_(True),
        #         weight_init=1
        #     )
        #     render_2 = run_rendering(
        #         albedo_gt[:,0].float().to(albedo_predict.device), 
        #         material_predict[0].repeat(3,1,1).float(), 
        #         material_predict[1].repeat(3,1,1).float(), 
        #         normal_predict[:,0].float(),
        ...
        #         weight_init=1
        #     )
        #     _ = torch.stack((render_1, render_2, render_3, render_4, render_5, render_6, render_7, render_8, render_9, render_10, render_11, render_12, render_13, render_14, render_15, render_16), dim=0)
        #     torchvision.utils.save_image(_, f"debug_render_{sigma}.png")
          
        # print((loss_render_albedo + loss_render_material + loss_render_normal) / 3)
        # print(loss)
    ...
    def from_pretrained(
        ...
        config_path = "configs/wan2_1_1_3b_t2v_dit_config.json"
        ckpt_path = "checkpoints/Wan2.1-T2V-1.3B/diffusion_pytorch_model.safetensors"
...
class WanTrainingModule_wan2_1_1_3b_t2v_pbrgen_pbr_sft_rec_gen_render_loss_v0_lora_image_train_v1(DiffusionTrainingModule): 
    def __init__(
        self,
        model_paths=None, model_id_with_origin_paths=None,
        trainable_models=None,
        lora_base_model=None, lora_target_modules="q,k,v,o,ffn.0,ffn.2", lora_rank=32, lora_modalities:list[str]=None,
        use_gradient_checkpointing=False, # 默认false好，xiuyu的经验 # 还是改成true吧，不然训不下
