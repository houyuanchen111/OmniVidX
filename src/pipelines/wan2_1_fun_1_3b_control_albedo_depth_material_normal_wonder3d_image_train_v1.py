...
只新加attention，其余全部不动
目的：
1. 尽可能保持diffusion prior
2. 我感觉这种结构是完全有潜力只用image就训好video的，简单做个实验看可不可以
""" 
...
from ..models.wan_video_dit_albedo_depth_material_normal_wonder3d_v1 import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        **kwargs 
        ...
        input_videos = torch.cat((data["normal"], data["depth"]), dim=0) # 2b c t h w, 先normal再depth
        ...
        video_depth = self.vae.decode(inputs_shared["latents"][[1]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, batch_size):
        ...
        noise_pair = pipe.generate_noise((4, 16, length, height//8, width//8), seed=seed, rand_device=rand_device)
        rep = (batch_size,) + (1,) * (noise_pair.ndim - 1)
        noise = torch.cat([
            noise_pair[0:1].repeat(*rep),
            noise_pair[1:2].repeat(*rep),
            noise_pair[2:3].repeat(*rep),
            noise_pair[3:4].repeat(*rep)
        ], dim=0)
        return {"noise": noise}
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, prompt, positive) -> dict:
        if isinstance(prompt, list):
            prompt = prompt * 4 # 这边在不同prompt的情况下要修改
...
class WanTrainingModule_wan2_1_fun_1_3b_control_albedo_depth_material_normal_wonder3d_image_train_v1(DiffusionTrainingModule): 
    def __init__(
        self,
    ...
    def forward_preprocess(self, data):
        ...
        # 在batch维度cat
        input_videos = torch.cat((data["albedo"], data["depth"], data["material"], data["normal"]), dim=0) # 4b c t h w
        inputs_shared["input_videos"] = [input_videos]
