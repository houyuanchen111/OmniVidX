"""
加入diffusion focing的训练方式试一下 (不同的模态使用不同的timestep)
""" 
...
from ..models.wan_video_dit_pbr_sft_rec_gen_wo_qk_lora import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        timestep_id_albedo = torch.randint(0, self.scheduler.num_train_timesteps, (1,)) # 越大越不加噪
        timestep_id_material = torch.randint(0, self.scheduler.num_train_timesteps, (1,)) # 越大越不加噪
        timestep_id_normal = torch.randint(0, self.scheduler.num_train_timesteps, (1,)) # 越大越不加噪
        timestep_weight_albedo = self.scheduler.timesteps[timestep_id_albedo].to(dtype=self.torch_dtype, device=self.device) # 越小越不加噪
        timestep_weight_material = self.scheduler.timesteps[timestep_id_material].to(dtype=self.torch_dtype, device=self.device) # 越小越不加噪
        timestep_weight_normal = self.scheduler.timesteps[timestep_id_normal].to(dtype=self.torch_dtype, device=self.device) # 越小越不加噪
        timestep = [
            self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
            timestep_weight_albedo,
            timestep_weight_material,
            timestep_weight_normal,
            ]
        ...
        if noise_pred.shape[0] == 4:
            loss = torch.nn.functional.mse_loss(noise_pred.float()[1:], training_target.float()[1:], reduction="none")
        elif noise_pred.shape[0] == 3:
            loss = torch.nn.functional.mse_loss(noise_pred.float(), training_target.float()[1:], reduction="none")
        else:
            print("noise_pred.shape[0] != 4 and noise_pred.shape[0] != 3")
        loss = loss[0].mean() * self.scheduler.training_weight(timestep_weight_albedo) + loss[1].mean() * self.scheduler.training_weight(timestep_weight_material) + loss[2].mean() * self.scheduler.training_weight(timestep_weight_normal)
        return loss / 3
    ...
    def __call__(
        ...
        **kwargs 
        ...
        inputs_shared["latents"][0], sigmas = self.scheduler.add_noise(inputs_shared["inference_image_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device)) # 
...
class WanTrainingModule_wan2_1_1_3b_t2v_pbrgen_pbr_sft_rec_gen_diffusion_forcing_wo_qk_lora_lora_image_train_v0(DiffusionTrainingModule): 
    def __init__(
        self,
