...
from ..models.wan_video_dit_matte_v1_single_lora import WanModel, RMSNorm, sinusoidal_embedding_1d
    ...
    def __call__(
        ...
        **kwargs 
        ...
        if training_mode == "RGBAGeneration":
            ...
            video_pha = self.vae.decode(inputs_shared["latents"], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video_pha = self.vae_output_to_video(video_pha, pattern="B C T H W")
            video_dict["pha"] = video_pha # torch.tensor[c,t,h,w]
        elif training_mode == "Matte":
            video_pha = self.vae.decode(inputs_shared["latents"][ [1] ], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device):
        ...
        noise = pipe.generate_noise((1, 16, length, height//8, width//8), seed=seed, rand_device=rand_device) # 三种模态的noise应该不一样
        return {"noise": noise}
...
class WanTrainingModule_wan2_1_14b_t2v_matte_lora_train_v6_single_lora(DiffusionTrainingModule): 
    def __init__(
        self,
