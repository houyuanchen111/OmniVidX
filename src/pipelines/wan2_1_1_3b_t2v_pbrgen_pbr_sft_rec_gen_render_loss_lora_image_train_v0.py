...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        try:
            inputs["latents"], sigmas = self.scheduler.add_noise(inputs["input_latents"], inputs["noise"], timestep) # 这个sigmas要小心
        except:
        ...
        if noise_pred.shape[0] == 4:
            ...
            sigma = sigmas[1].item()
            z0_predict = inputs["latents"][1:] - sigma * noise_pred[1:] # z0_predict = zt - sigma * (noise_pred - z0_predict)
            # import torchvision
            # torchvision.utils.save_image(torch.cat((inputs["input_latents"][1:], z0_predict), dim=0)[:,:3,0].clip(0,1), "debug_rec.png")
            loss_rec = torch.nn.functional.mse_loss(inputs["input_latents"][1:].float(), z0_predict.float())
        elif noise_pred.shape[0] == 3:
            loss = torch.nn.functional.mse_loss(noise_pred.float(), training_target.float()[1:])
            sigma = sigmas[1].item()
            z0_predict = inputs["latents"][1:] - sigma * noise_pred # z0_predict = zt - sigma * (noise_pred - z0_predict)
            # import torchvision
            # torchvision.utils.save_image(torch.cat((inputs["input_latents"][1:], z0_predict), dim=0)[:,:3,0].clip(0,1), "debug_rec.png")
            loss_rec = torch.nn.functional.mse_loss(inputs["input_latents"][1:].float(), z0_predict.float())
        else:
            print("noise_pred.shape[0] != 4 and noise_pred.shape[0] != 3")
        loss = (loss + loss_rec) * self.scheduler.training_weight(timestep_weight) 
        return loss
    ...
    def __call__(
        ...
        **kwargs 
        ...
        try:
            ...
            inputs_shared["latents"][0] = self.scheduler.add_noise(inputs_shared["inference_image_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device)) # 
...
class WanTrainingModule_wan2_1_1_3b_t2v_pbrgen_pbr_sft_rec_gen_render_loss_lora_image_train_v0(DiffusionTrainingModule): 
    def __init__(
        self,
