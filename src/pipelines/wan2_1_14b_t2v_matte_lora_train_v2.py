...
from ..models.wan_video_dit_matte_v1 import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        # Matte任务的四个分支: [com, pha, fgr, bgr]
        if training_mode == "RGBAGeneration":
            timestep = [
                timestep_weight,
                timestep_weight,
                timestep_weight,
            ...
            timestep = [
                ...
                timestep_weight,
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                ] # 修复任务: pha和fgr branch timestep为999，预测 com 和 bgr
        elif training_mode == "BackgroundGeneration":
            timestep = [
                timestep_weight,
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                timestep_weight,
                ] # 背景生成: pha和fgr branch timestep为999，预测 com 和 bgr
        ...
        if training_mode == "RGBAGeneration":
            ...
            # 只计算 pha, fgr, bgr 三个分支的loss (索引1,2,3)
            loss = torch.nn.functional.mse_loss(noise_pred.float()[1:], training_target.float()[1:])
        elif training_mode == "Inpainting":
            # 只计算 com 和 fgr 两个分支的loss (索引0,2)
            loss = torch.nn.functional.mse_loss(noise_pred.float()[[0,2]], training_target.float()[[0,3]])
        elif training_mode == "BackgroundGeneration":
            # 只计算 com 和 bgr 两个分支的loss (索引0,3)
    ...
    def __call__(
        ...
        **kwargs 
        ...
        # Matte任务的推理初始化: [com, pha, fgr, bgr]
        if training_mode == "Matte":
            # 抠图任务: com分支条件固定，预测 pha, fgr, bgr
            inputs_shared["latents"][0], sigmas = self.scheduler.add_noise(inputs_shared["inference_com_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device))
        elif training_mode == "Inpainting":
            # 修复任务: pha和fgr分支条件固定，预测 com 和 bgr
            ...
            # 背景生成: pha和fgr分支条件固定，预测 com 和 bgr
            inputs_shared["latents"][1], sigmas = self.scheduler.add_noise(inputs_shared["inference_pha_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device))
        ...
        for progress_id, timestep in enumerate(tqdm(scheduler_copy.timesteps)):
            ...
            # Matte任务的推理timestep设置: [com, pha, fgr, bgr]
            if training_mode == "RGBAGeneration":
                timestep = [
                    timestep,
                    timestep,
                ...
                # 抠图任务: com分支固定timestep=999，预测 pha, fgr, bgr
                timestep = [
                    self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                    timestep,
                    timestep,
                ...
                # 修复任务: pha和fgr分支固定timestep=999，预测 com 和 bgr
                timestep = [
                    timestep,
                    self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                    timestep,
                ...
                # 背景生成: pha和fgr分支固定timestep=999，预测 com 和 bgr
                timestep = [
                    timestep,
                    self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
                    self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device),
        ...
        # Decode - Matte任务的解码: [com, pha, fgr, bgr]
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    # Matte任务的输入编码: [com, pha, fgr, bgr]
    def __init__(self):
        super().__init__(
            input_params=("input_videos", "noise", "tiled", "tile_size", "tile_stride", "is_inference", "modality_index", "inference_com", "inference_pha", "inference_fgr", "inference_bgr", "height", "width", "training_mode"),
            onload_model_names=("vae",)
        )
    def process(self, pipe: WanVideoPipeline, input_videos, noise, tiled, tile_size, tile_stride, is_inference, modality_index, inference_com, inference_pha, inference_fgr, inference_bgr, height, width, training_mode):
        if is_inference:
...
class WanTrainingModule_wan2_1_14b_t2v_matte_lora_train_v2(DiffusionTrainingModule): 
    def __init__(
        self,
    ...
    def training_mode(self) -> str:
        ...
        if dist.is_available() and dist.is_initialized():
            ...
            if dist.get_rank() == 0:
                ...
                if flag < 0.25:
                    mode_idx = 0  # "RGBAGeneration"
                elif flag < 0.5:
                    mode_idx = 1  # "Matte"
                elif flag < 0.75:
                    mode_idx = 2  # "Inpainting"
                else:
                    mode_idx = 3  # "BackgroundGeneration"
            else:
            ...
            # 映射回字符串
            modes = ["RGBAGeneration", "Matte", "Inpainting", "BackgroundGeneration"]
            return modes[mode_idx]
        else:
            # 单卡训练时的原有逻辑
            flag = torch.rand(1).item()
            if flag < 0.25:
                return "RGBAGeneration"
            elif flag < 0.5:
                return "Matte"
            elif flag < 0.75:
                return "Inpainting"
    ...
    def forward_preprocess(self, data):
        ...
        # if inputs_shared["training_mode"] == "Matte":
        #     inputs_posi = {"prompt": ""} # 抠图不需要
        # elif inputs_shared["training_mode"] == "RGBAGeneration":
        #     inputs_posi = {"prompt": data["prompt"]} # 其他都需要
        # elif inputs_shared["training_mode"] == "Inpainting":
        #     inputs_posi = {"prompt": data["fgr_prompt"]}
        # elif inputs_shared["training_mode"] == "BackgroundGeneration":
        #     inputs_posi = {"prompt": data["bg_prompt"]}
        if inputs_shared["training_mode"] == "Matte":
            inputs_posi = {"prompt": ""} # 抠图不需要
        else:
            inputs_posi = {"prompt": data["prompt"]} # 其他都需要
