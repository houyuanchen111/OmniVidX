"""
尝试使用gradient checkpoint技术， 把decode放到gpu上
""" 
import torchvision
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            seperate_cfg=True, # 
            input_params_posi={"prompt": "prompt", "positive": "positive", "scene_name": "scene_name"},
            input_params_nega={"prompt": "negative_prompt", "positive": "positive", "scene_name": "scene_name"},
            onload_model_names=("text_encoder",)
        )

    def process(self, pipe: WanVideoPipeline, prompt, positive, scene_name) -> dict:
        pipe.load_models_to_device(self.onload_model_names)
        prompt_emb = pipe.prompter.encode_prompt(prompt, positive=positive, device=str(pipe.device)) # 1 512 4096
        
        #prompt_emb = pipe.prompter.encode_prompt(prompt, positive=positive, device=str(pipe.device)) # 1 512 4096
        torch.save({
                'embedding': prompt_emb,
                'shape': prompt_emb.shape,
                'dtype': prompt_emb.dtype
            }, f"prompt_embedding/{scene_name[0]}.pth")
...
class WanTrainingModule_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_rm_rgblora_render_loss_v1_lora_video_save_prompt_embedding_v2(DiffusionTrainingModule): 
    def __init__(
        self,
    ...
    def forward_preprocess(self, data):
        ...
        if not inputs_shared["is_generation"]:
            inputs_posi = {"prompt": data["prompt"], "scene_name": data["scene_name"]} 
        else:
            inputs_posi = {"prompt": data["prompt"], "scene_name": data["scene_name"]}
        # 只试试albedo的效果
