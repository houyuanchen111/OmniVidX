"""
实验4
在实验三的基础上，再给patch embed 加上 zero init residual conv
""" 
...
from ..models.wan_video_dit_wonder3d_v4 import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        **kwargs 
        ...
        video_normal = self.vae.decode(inputs_shared["latents"][[0]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, batch_size):
        ...
        noise_pair = pipe.generate_noise((2, 16, length, height//8, width//8), seed=seed, rand_device=rand_device)
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, prompt, positive) -> dict:
        if isinstance(prompt, list):
            prompt = prompt * 2 # 这边在不同prompt的情况下要修改
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # 为不同模态（前半: normal，后半: depth）添加可学习的 domain embedding
    # x: (B, L, C)
    B = x.shape[0]
    if B % 2 == 0 and hasattr(dit, "domain_embedding"):
        domain_ids = torch.zeros(B, dtype=torch.long, device=x.device)
        domain_ids[B // 2:] = 1  # 后半 batch 设为 1
...
class WanTrainingModule_wan2_1_fun_1_3b_control_normal_depth_wonder3d_image_train_v4(DiffusionTrainingModule): 
    def __init__(
        self,
    ...
    def forward_preprocess(self, data):
        ...
        for extra_input in self.extra_inputs:
            if extra_input == "input_image":
                ...
                inputs_shared[extra_input] = torch.cat((data["rgb"], data["rgb"]), dim=0) # 2b c t h w
            else:
