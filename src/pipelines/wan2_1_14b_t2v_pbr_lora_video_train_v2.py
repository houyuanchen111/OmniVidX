...
from ..models.wan_video_dit_pbr_video_v1 import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        if training_mode == "PBRGeneration":
            timestep = [
                timestep_weight,
                timestep_weight,
                timestep_weight,
        ...
        elif training_mode == "Inverse_Rendering":
            timestep = [
        ...
        if training_mode == "PBRGeneration":
            ...
            if dataset_name and "hypersim" in dataset_name:
                # hypersim 数据集在 PBRGeneration 模式下，loss 设为 0（不影响梯度回传）
                print(f"✅ hypersim 数据集在 PBRGeneration 模式下，loss 设为 0（不影响梯度回传）")
    ...
    def __call__(
        ...
        **kwargs 
        ...
        if training_mode == "Inverse_Rendering":
            ...
            inputs_shared["latents"][1], sigmas = self.scheduler.add_noise(inputs_shared["inference_albedo_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device)) # 替换成rgb cond
            inputs_shared["latents"][3], sigmas = self.scheduler.add_noise(inputs_shared["inference_normal_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device)) # 替换成rgb cond
        elif training_mode == "Reconstruction":
            inputs_shared["latents"][1], sigmas = self.scheduler.add_noise(inputs_shared["inference_albedo_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device)) # 替换成rgb cond
            inputs_shared["latents"][2], sigmas = self.scheduler.add_noise(inputs_shared["inference_irradiance_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device)) # 替换成rgb cond
            inputs_shared["latents"][3], sigmas = self.scheduler.add_noise(inputs_shared["inference_normal_latents"], noise_0, self.scheduler.timesteps[torch.tensor([999])].to(dtype=self.torch_dtype, device=self.device)) # 替换成rgb cond
        ...
        if is_inference:
            if training_mode == "PBRGeneration":
                ...
                if inference_albedo is not None and isinstance(inference_albedo, Image.Image):
                    ...
                    inference_albedo_latents = pipe.vae.encode(inference_albedo, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
                return {"latents": noise, "inference_normal_latents": inference_normal_latents, "inference_albedo_latents": inference_albedo_latents}
            elif training_mode == "Reconstruction":
                if inference_albedo is not None and isinstance(inference_albedo, Image.Image):
                    pipe.load_models_to_device(["vae"])
                    inference_albedo = pipe.preprocess_image(inference_albedo.resize((width, height))).to(pipe.device).to(pipe.torch_dtype)[:,:,None]
...
class WanTrainingModule_wan2_1_14b_t2v_pbr_lora_video_train_v2(DiffusionTrainingModule): 
    def __init__(
        self,
    ...
    def training_mode(self) -> str:
        ...
        if dist.is_available() and dist.is_initialized():
            ...
            if dist.get_rank() == 0:
                ...
                if flag < 1/4:
                    mode_idx = 0  # "PBRGeneration"
                elif flag < 2/4:
                    mode_idx = 1  # "Inverse_Rendering"
                elif flag < 3/4:
                    mode_idx = 2  # "Relighting"
                else:
                    mode_idx = 3  # "Reconstruction"
            else:
            ...
            # 映射回字符串
            modes = ["PBRGeneration", "Inverse_Rendering", "Relighting", "Reconstruction"]
            return modes[mode_idx]
        else:
            # 单卡训练时的原有逻辑
            flag = torch.rand(1).item()
            if flag < 1/4:
                return "PBRGeneration"
            elif flag < 2/4:
                return "Inverse_Rendering"
            elif flag < 3/4:
                return "Relighting"
            else:
    ...
    def forward_preprocess(self, data):
        ...
        if inputs_shared["training_mode"] == "PBRGeneration":
            prompt_list = [
                data["prompt"][0],
                data["prompt"][0],
                data["prompt"][0],
        ...
        elif inputs_shared["training_mode"] == "Inverse_Rendering":
            prompt_list = [
                "",
                "",
                "",
            ...
            inputs_posi = {"prompt": prompt_list}
            # print("when preparing inputs, training_mode == Inverse_Rendering")
        elif inputs_shared["training_mode"] == "Reconstruction":
            prompt_list = [
                "",
                "",
                "",
        ...
        elif inputs_shared["training_mode"] == "Relighting":
            prompt_list = [
                data["prompt"][0],
                data["prompt"][0],
                data["prompt"][0],
