...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        mode_names = ["normal", "roughness", "metallic", "albedo"]
        sigma = sigmas[1].item()
        ...
        if render_mode == 0:
            # 只使用normal_predict
            normal_predict = self.vae.decode(
                z0_predict_frame_0[[2]].cpu(), 
                device='cpu',
                tiled=inputs["tiled"], 
                tile_size=inputs["tile_size"], 
                tile_stride=inputs["tile_stride"]
            )[0].to(cuda_device)
            render_predict = run_rendering(
                albedo_gt[:,0].float().to(cuda_device), 
                material_gt[0,0].repeat(3,1,1).float().to(cuda_device), 
                material_gt[1,0].repeat(3,1,1).float().to(cuda_device), 
                normal_predict[:,0].float(), # normal就是-1 -1之间就行
                lighting_longitude = torch.tensor(longtitude).to(cuda_device).float().requires_grad_(True), 
                lighting_latitude = torch.tensor(latitude).to(cuda_device).float().requires_grad_(True), 
                weight_init=weight_init
            )
            ...
            material_predict = self.vae.decode(
                z0_predict_frame_0[[1]].cpu(), 
                device='cpu',
                tiled=inputs["tiled"], 
                tile_size=inputs["tile_size"], 
                tile_stride=inputs["tile_stride"]
            ...
            render_predict = run_rendering(
                albedo_gt[:,0].float().to(cuda_device), 
                material_gt[0,0].repeat(3,1,1).float().to(cuda_device), 
                material_predict[1].repeat(3,1,1).float() * 0.5 + 0.5, 
                normal_gt[:,0].float().to(cuda_device),
                lighting_longitude = torch.tensor(longtitude).to(cuda_device).float().requires_grad_(True), 
                lighting_latitude = torch.tensor(latitude).to(cuda_device).float().requires_grad_(True), 
                weight_init=weight_init
            ...
            render_predict = run_rendering(
                albedo_gt[:,0].float().to(cuda_device), 
                material_predict[0].repeat(3,1,1).float() * 0.5 + 0.5, 
                material_gt[1,0].repeat(3,1,1).float().to(cuda_device), 
                normal_gt[:,0].float().to(cuda_device),
                lighting_longitude = torch.tensor(longtitude).to(cuda_device).float().requires_grad_(True), 
                lighting_latitude = torch.tensor(latitude).to(cuda_device).float().requires_grad_(True), 
            ...
            # 只使用albedo_predict
            albedo_predict = self.vae.decode(
                z0_predict_frame_0[[0]].cpu(), 
                device='cpu',
                tiled=inputs["tiled"], 
                tile_size=inputs["tile_size"], 
                tile_stride=inputs["tile_stride"]
            ...
            render_predict = run_rendering(
                albedo_predict[:,0].float() * 0.5 + 0.5, 
                material_gt[0,0].repeat(3,1,1).float().to(cuda_device), 
                material_gt[1,0].repeat(3,1,1).float().to(cuda_device), 
                normal_gt[:,0].float().to(cuda_device),
                lighting_longitude = torch.tensor(longtitude).to(cuda_device).float().requires_grad_(True), 
...
class WanTrainingModule_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_rm_rgblora_render_loss_v1_lora_video_train_v1(DiffusionTrainingModule): 
    def __init__(
        self,
