"""
v3: cat on the channel dim depth and normal, different timestep
""" 
    ...
    def __call__(
        ...
        **kwargs 
        ...
        if video_dict["context_index"].item() == 1:
            video_disparity = self.vae.decode(inputs_shared["latents"], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def __init__(self):
        super().__init__(
            input_params=("normals", "disparities", "noise", "tiled", "tile_size", "tile_stride", "is_inference", "context_index"),
            onload_model_names=("vae",)
        )
    def process(self, pipe: WanVideoPipeline, normals, disparities, noise, tiled, tile_size, tile_stride, is_inference, context_index):
        if is_inference:
        ...
        if context_index.item() == 0:
            ...
            raise ValueError(f"context_index must be 0 or 1, but got {context_index}")
...
class WanVideoUnit_Modality_Embedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline,):
        if random.random() < 0.5:
            context_index = torch.tensor([[0]], device=pipe.device)
        else:
            context_index = torch.tensor([[1]], device=pipe.device)
        return {"context_index": context_index}
...
class WanTrainingModule_wan2_1_fun_1_1_1_3b_control_normal_depth_train_v0(DiffusionTrainingModule): 
    def __init__(
        self,
        ...
        self.pipe.dit.context_embedding = torch.nn.Embedding(16, 1536) # follow diffusion renderer
    ...
    def forward_preprocess(self, data):
        ...
        inputs_shared = {
            ...
            "disparities": data.get("disparities", None), # b c t h w，需要仔细确认加载是否正确
            "height": data["height"],
            "width": data["width"],
