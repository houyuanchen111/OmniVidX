...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        timestep = self.scheduler.timesteps[timestep_id].to(dtype=self.torch_dtype, device=self.device)
         # 逻辑在这里发生一些变动，目的是将控制信号转向cat于frame dim
    ...
    def __call__(
        ...
        **kwargs 
        ...
        # 输入模态
        input_videos = inputs_shared.get("input_videos", None)
        reconstructed_videos = {}
        if input_videos is not None:
            for key, input_video in input_videos.items():
                latents = self.vae.encode(input_video, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
                reconstructed_video = self.vae.decode(latents, device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
                reconstructed_videos[key] = reconstructed_video
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    # 适配recammaster的逻辑，核心是(num_input_videos + 1)
    def __init__(self):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, num_input_videos):
        ...
        noise = pipe.generate_noise((1, 16, length* (num_input_videos + 1), height//8, width//8), seed=seed, rand_device=rand_device)
        return {"noise": noise}
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_videos: list, noise, tiled, tile_size, tile_stride):
        if input_videos is None:
            return {"latents": noise} # 如果没有input_video，则直接从noise出发; noise和latent的shape相同；这一行是需要的
        else:
            ...
            input_latents = torch.cat(input_latents_list, dim=1) # cat 在 C，list 只有一个也不报错
            if pipe.scheduler.training:
                return {"latents": noise, "input_latents": input_latents}
            else: # 这一行是为了video2video（指的是input_video）准备的，而我们的是control video，逻辑不一样，所以这一行不合理 TODO inference 改正这里的逻辑
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if reference_latents is not None:
        ...
        f -= 1
    x = dit.unpatchify(x, (f, h, w)) # type: ignore
    return x

# class WanTrainingModule_wan2_1_t2v_1_3b_control_normal_train_po_v0(DiffusionTrainingModule):
#     def __init__(
#         self,
...
#             model_configs += [ModelConfig(mokdel_id=i.split(":")[0], origin_file_pattern=i.split(":")[1]) for i in model_id_with_origin_paths]
