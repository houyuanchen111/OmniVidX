...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        **kwargs 
        ...
        B, C, T, H, W = inputs_shared["input_latents"].shape
        for progress_id, timestep in enumerate(tqdm(scheduler_copy.timesteps)):
            timestep = timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device)
            inputs_shared["latents"][:,:,T:] = inputs_shared["control_latents"] # cond 不加噪声
        ...
        video_dict = {}
        # 反正要重写，写死一点
        video = self.vae.decode(inputs_shared["latents"][:, :, :T,], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video_normal_unit= self.vae_normal_output_to_video(video) # tensor 2 Image
...
class WanTrainingModule_wan2_1_t2v_1_3b_control_normal_lora_train_po_v0(DiffusionTrainingModule):
    def __init__(
        self,
        ...
        print(f"lora 总可训练参数量: {total_params/1024/1024:.2f} MB; lora rank: {lora_rank}")
