...
class ModelCheckpointCallback_wan2_1_1_3b_t2v_pbrgen_pbr_sft_rec_gen_lora_image_train_v1(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_1_3b_t2v_pbrgen_pbr_sft_rec_gen_lora_image_train_v1(Callback):    
...
class Trainer_wan2_1_1_3b_t2v_pbrgen_pbr_sft_rec_gen_lora_image_train_v1:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        total_loss = 0
        count = 0
        inference_image = Image.open("debug.png")
        
        # rec的validate

        inference_params = {
            # "prompt":"纪实摄影风格画面，一只活泼的小狗在绿茵茵的草地上迅速奔跑。小狗毛色棕黄，两只耳朵立起，神情专注而欢快。阳光洒在它身上，使得毛发看上去格外柔软而闪亮。背景是一片开阔的草地，偶尔点缀着几朵野花，远处隐约可见蓝天和几片白云。透视感鲜明，捕捉小狗奔跑时的动感和四周草地的生机。中景侧面移动视角。",
            "prompt":"国家地理纪录片风格，写实，手持摄像机视角，一个身穿带有磨损痕迹的NASA标准宇航服的宇航员，正努力骑在一匹肌肉发达、毛色纯白的独角兽上。",
        ...
        self._tensor2video(albedo_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/albedo_rec_{count}.mp4")
        roughness_gen = video_dict["roughness"]
        self._tensor2video(roughness_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/roughness_rec_{count}.mp4")
        metallic_gen = video_dict["metallic"]
        self._tensor2video(metallic_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/metallic_rec_{count}.mp4")
        normal_gen = video_dict["normal_unit"]
        self._tensor2video(normal_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/normal_rec_{count}.mp4")

        # 保存cat在w维度的综合视频
        # 将所有模态沿着宽度维度拼接
        modalities_to_cat = [
            albedo_gen * 0.5 + 0.5,
            roughness_gen * 0.5 + 0.5,
            metallic_gen * 0.5 + 0.5,
            normal_gen * 0.5 + 0.5
        ]
        im = (torch.from_numpy(np.array(inference_image.resize((640,480)))) / 255.0).to(albedo_gen.device).to(albedo_gen.dtype).permute(2,0,1)[:,None]
        cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接 [c, t, h, w*4]
        cat_video = torch.cat((im, cat_video), dim=-1)
        # self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/cat_all_modalities_rec_{count}.mp4")
        torchvision.utils.save_image(cat_video[:,0], f"{self.output_path}/epoch_{self.epoch_id}/cat_all_modalities_rec_{count}.png")
        ...
        inference_params = {
            ...
            "tile_stride": [15, 26],
            "is_inference": True,
            # "inference_image": inference_image, # 生成的时候不使用inference_image
            "is_generation": True,
        }   
        ...
        self._tensor2video(rgb_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/rgb_gen_{count}.mp4")
        albedo_gen = video_dict["albedo"]
        self._tensor2video(albedo_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/albedo_gen_{count}.mp4")
        roughness_gen = video_dict["roughness"]
        ...
        normal_gen = video_dict["normal_unit"]
        self._tensor2video(normal_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/normal_gen_{count}.mp4")
        ...
        # self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/cat_all_modalities_gen_{count}.mp4")
        torchvision.utils.save_image(cat_video[:,0], f"{self.output_path}/epoch_{self.epoch_id}/cat_all_modalities_gen_{count}.png")
    ...
    def train(self):
        ...
        for epoch_id in range(self.num_epochs):
            ...
            if self.num_epochs == 100:
                ...
                if self.epoch_id % 25 == 0: # 每500 steps做一次验证
                    if self.accelerator.is_main_process:
                        try:
                            self.val_loss = self.validate() # type: ignore
                        except Exception as e:
