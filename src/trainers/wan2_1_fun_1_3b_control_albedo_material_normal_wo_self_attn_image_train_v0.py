"""
这边是wan2.1-t2v-1.3b-control-albedo-material-normal-train的trainer
可训练self.attn
"""
...
class ModelCheckpointCallback_wan2_1_fun_1_3b_control_albedo_material_normal_wo_self_attn_image_train_v0(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_fun_1_3b_control_albedo_material_normal_wo_self_attn_image_train_v0(Callback):    
...
class Trainer_wan2_1_fun_1_3b_control_albedo_material_normal_wo_self_attn_image_train_v0:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        inference_params = {
            "prompt": 
            ["Generate a video showing realistic surface coloring and appearance of objects",
        ...
        with torch.no_grad():
            for data in tqdm(self.val_dataloader, disable=not self.accelerator.is_main_process, desc=f"Epoch {self.epoch_id}: Validating"):
                ...
                inference_params["control_video"] = torch.cat((data["rgb"], data["rgb"], data["rgb"]), dim=0).to(self.model.pipe.device).to(self.model.pipe.torch_dtype) # 3b
                ...
                try:
                    ...
                    material_loss = self._compute_modality_loss(
                            video_dict, data, 1, mask, normal_gt, norm_normal_gt
                        )
                    normal_loss = self._compute_modality_loss(
                            video_dict, data, 2, mask, normal_gt, norm_normal_gt
                        )
                    total_loss += (albedo_loss + material_loss + normal_loss)
                    epoch_losses[f"modality_{0}"] = albedo_loss.item()
                    epoch_losses[f"modality_{1}"] = material_loss.item()
                    epoch_losses[f"modality_{2}"] = normal_loss.item()
                    self._save_validation_videos(
                        video_dict, data, rgbs_input_cond, epoch_losses, count, 0
                    )
                    self._save_validation_videos(
                        video_dict, data, rgbs_input_cond, epoch_losses, count, 1
                    )
                    self._save_validation_videos(
    ...
    def _compute_modality_loss(self, video_dict, data, modality_idx, mask, normal_gt, norm_normal_gt):
        ...
        MODALITY_CONFIG = {
            ...
            1: {"key": "material", "name": "material"},
            2: {"key": "normal_unit", "name": "normal", "special": "normal"}
        }
    ...
    def _save_validation_videos(self, video_dict, data, rgbs_input_cond, epoch_losses, count, modality_idx):
        ...
        MODALITY_CONFIG = {
            ...
            1: {"name": "material", "type": "dual", "keys": ["roughness", "metallic"]},
            2: {"name": "normal", "type": "single", "special": "normal"}
        }
