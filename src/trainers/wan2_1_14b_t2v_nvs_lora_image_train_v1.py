...
class ModelCheckpointCallback_wan2_1_14b_t2v_nvs_lora_image_train_v1(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_14b_t2v_nvs_lora_image_train_v1(Callback):    
...
class Trainer_wan2_1_14b_t2v_nvs_lora_image_train_v1:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        # 提取 NVS 任务的数据: [cam1, cam2, cam3, cam4]
        inference_cam1 = val_batch.get("cam1")
        inference_cam2 = val_batch.get("cam2")
        inference_cam3 = val_batch.get("cam3")
        inference_cam4 = val_batch.get("cam4")
        
        self.model.eval()
        count = 0
        
        # ========== 1. Generation: 全生成模式 (生成所有4个相机视角，保存为PNG) ==========
        print("\n[Validation] Testing Generation mode...")
        inference_params = {
            "prompt":"一个小狗在草地上快乐的奔跑。 ",
        ...
        # 保存各个相机的生成图片（取第1帧）
        if "cam1" in video_dict:
            cam1_img = (video_dict["cam1"] * 0.5 + 0.5)[:,0]
            torchvision.utils.save_image(cam1_img, f"{self.output_path}/epoch_{self.epoch_id}/Generation/cam1_gen_{count}.png")
        if "cam2" in video_dict:
            cam2_img = (video_dict["cam2"] * 0.5 + 0.5)[:,0]
            torchvision.utils.save_image(cam2_img, f"{self.output_path}/epoch_{self.epoch_id}/Generation/cam2_gen_{count}.png")
        if "cam3" in video_dict:
            cam3_img = (video_dict["cam3"] * 0.5 + 0.5)[:,0]
            torchvision.utils.save_image(cam3_img, f"{self.output_path}/epoch_{self.epoch_id}/Generation/cam3_gen_{count}.png")
        if "cam4" in video_dict:
            cam4_img = (video_dict["cam4"] * 0.5 + 0.5)[:,0]
            torchvision.utils.save_image(cam4_img, f"{self.output_path}/epoch_{self.epoch_id}/Generation/cam4_gen_{count}.png")

        # 保存拼接图片（横向拼接所有相机）
        cameras_to_cat = []
        if "cam1" in video_dict:
            cameras_to_cat.append((video_dict["cam1"] * 0.5 + 0.5)[:,0])
        if "cam2" in video_dict:
            cameras_to_cat.append((video_dict["cam2"] * 0.5 + 0.5)[:,0])
        if "cam3" in video_dict:
            cameras_to_cat.append((video_dict["cam3"] * 0.5 + 0.5)[:,0])
        if "cam4" in video_dict:
            cameras_to_cat.append((video_dict["cam4"] * 0.5 + 0.5)[:,0])
        
        if cameras_to_cat:
            cat_image = torch.cat(cameras_to_cat, dim=-1)  # 在宽度维度拼接
            torchvision.utils.save_image(cat_image, f"{self.output_path}/epoch_{self.epoch_id}/Generation/cat_all_{count}.png")
 
        # ========== 2. cam1: 以cam1为条件，预测cam2, cam3, cam4（保存为PNG） ==========
        print("\n[Validation] Testing cam1 mode...")
        if inference_cam1 is not None:
            inference_params = {
                "prompt": "",  # cam1 条件任务不需要 prompt
                "negative_prompt": "色调艳丽，过曝，静态，细节模糊不清，字幕，风格，作品，画作，画面，静止，整体发灰，最差质量，低质量，JPEG压缩残留，丑陋的，残缺的，多余的手指，画得不好的手部，画得不好的脸部，畸形的，毁容的，形态畸形的肢体，手指融合，静止不动的画面，杂乱的背景，三条腿，背景人很多，倒着走",
                "seed": 1,
                "num_inference_steps": 50,
                "cfg_scale": 1.0,  # 条件任务通常使用较小的 CFG
                "cfg_merge": False,
                "height": 432,
                "width": 768,
                "num_frames": 1,
            ...
            video_dict = self.model.pipe(**inference_params)
            os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/cam1", exist_ok=True)
            
            # 保存预测图片
            if "cam2" in video_dict:
                cam2_img = (video_dict["cam2"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam2_img, f"{self.output_path}/epoch_{self.epoch_id}/cam1/cam2_gen_{count}.png")
            if "cam3" in video_dict:
                cam3_img = (video_dict["cam3"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam3_img, f"{self.output_path}/epoch_{self.epoch_id}/cam1/cam3_gen_{count}.png")
            if "cam4" in video_dict:
                cam4_img = (video_dict["cam4"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam4_img, f"{self.output_path}/epoch_{self.epoch_id}/cam1/cam4_gen_{count}.png")

            # 保存拼接图片 (输入cam1 + 预测的cam2, cam3, cam4)
            cameras_to_cat = [(inference_cam1[0] * 0.5 + 0.5)[:,0]]  # [C, H, W]
            if "cam2" in video_dict:
                cameras_to_cat.append((video_dict["cam2"] * 0.5 + 0.5)[:,0])
            if "cam3" in video_dict:
                cameras_to_cat.append((video_dict["cam3"] * 0.5 + 0.5)[:,0])
            ...
            cat_image = torch.cat(cameras_to_cat, dim=-1)  # 在宽度维度拼接
            torchvision.utils.save_image(cat_image, f"{self.output_path}/epoch_{self.epoch_id}/cam1/cat_all_{count}.png")
            
        # ========== 3. cam2: 以cam2为条件，预测cam1, cam3, cam4（保存为PNG） ==========
        print("\n[Validation] Testing cam2 mode...")
        if inference_cam2 is not None:
            inference_params = {
                ...
                "training_mode": "cam2",
                "inference_cam1": None,
                "inference_cam2": inference_cam2,
                "inference_cam3": None,
                "inference_cam4": None,
            }   

            video_dict = self.model.pipe(**inference_params)
            os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/cam2", exist_ok=True)
            
            # 保存预测图片
            if "cam1" in video_dict:
                cam1_img = (video_dict["cam1"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam1_img, f"{self.output_path}/epoch_{self.epoch_id}/cam2/cam1_gen_{count}.png")
            if "cam3" in video_dict:
                cam3_img = (video_dict["cam3"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam3_img, f"{self.output_path}/epoch_{self.epoch_id}/cam2/cam3_gen_{count}.png")
            if "cam4" in video_dict:
                cam4_img = (video_dict["cam4"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam4_img, f"{self.output_path}/epoch_{self.epoch_id}/cam2/cam4_gen_{count}.png")

            # 保存拼接图片
            cameras_to_cat = []
            if "cam1" in video_dict:
                cameras_to_cat.append((video_dict["cam1"] * 0.5 + 0.5)[:,0])
            cameras_to_cat.append((inference_cam2[0] * 0.5 + 0.5)[:,0])
            if "cam3" in video_dict:
                cameras_to_cat.append((video_dict["cam3"] * 0.5 + 0.5)[:,0])
            if "cam4" in video_dict:
                cameras_to_cat.append((video_dict["cam4"] * 0.5 + 0.5)[:,0])
            
            cat_image = torch.cat(cameras_to_cat, dim=-1)
            torchvision.utils.save_image(cat_image, f"{self.output_path}/epoch_{self.epoch_id}/cam2/cat_all_{count}.png")
            
        # ========== 4. cam3: 以cam3为条件，预测cam1, cam2, cam4（保存为PNG） ==========
        print("\n[Validation] Testing cam3 mode...")
        if inference_cam3 is not None:
            inference_params = {
                ...
                "inference_cam1": None,
                "inference_cam2": None,
                "inference_cam3": inference_cam3,
                "inference_cam4": None,
            }   

            video_dict = self.model.pipe(**inference_params)
            os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/cam3", exist_ok=True)
            
            # 保存预测图片
            if "cam1" in video_dict:
                cam1_img = (video_dict["cam1"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam1_img, f"{self.output_path}/epoch_{self.epoch_id}/cam3/cam1_gen_{count}.png")
            if "cam2" in video_dict:
                cam2_img = (video_dict["cam2"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam2_img, f"{self.output_path}/epoch_{self.epoch_id}/cam3/cam2_gen_{count}.png")
            if "cam4" in video_dict:
                cam4_img = (video_dict["cam4"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam4_img, f"{self.output_path}/epoch_{self.epoch_id}/cam3/cam4_gen_{count}.png")
            ...
            if "cam2" in video_dict:
                cameras_to_cat.append((video_dict["cam2"] * 0.5 + 0.5)[:,0])
            cameras_to_cat.append((inference_cam3[0] * 0.5 + 0.5)[:,0])
            if "cam4" in video_dict:
                cameras_to_cat.append((video_dict["cam4"] * 0.5 + 0.5)[:,0])
            
            cat_image = torch.cat(cameras_to_cat, dim=-1)
            torchvision.utils.save_image(cat_image, f"{self.output_path}/epoch_{self.epoch_id}/cam3/cat_all_{count}.png")
            
        # ========== 5. cam4: 以cam4为条件，预测cam1, cam2, cam3（保存为PNG） ==========
        print("\n[Validation] Testing cam4 mode...")
        if inference_cam4 is not None:
            inference_params = {
                ...
                "inference_cam2": None,
                "inference_cam3": None,
                "inference_cam4": inference_cam4,
            }   

            video_dict = self.model.pipe(**inference_params)
            os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/cam4", exist_ok=True)
            
            # 保存预测图片
            if "cam1" in video_dict:
                cam1_img = (video_dict["cam1"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam1_img, f"{self.output_path}/epoch_{self.epoch_id}/cam4/cam1_gen_{count}.png")
            if "cam2" in video_dict:
                cam2_img = (video_dict["cam2"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam2_img, f"{self.output_path}/epoch_{self.epoch_id}/cam4/cam2_gen_{count}.png")
            if "cam3" in video_dict:
                cam3_img = (video_dict["cam3"] * 0.5 + 0.5)[:,0]
                torchvision.utils.save_image(cam3_img, f"{self.output_path}/epoch_{self.epoch_id}/cam4/cam3_gen_{count}.png")
            ...
            if "cam3" in video_dict:
                cameras_to_cat.append((video_dict["cam3"] * 0.5 + 0.5)[:,0])
            cameras_to_cat.append((inference_cam4[0] * 0.5 + 0.5)[:,0])
            
            cat_image = torch.cat(cameras_to_cat, dim=-1)
            torchvision.utils.save_image(cat_image, f"{self.output_path}/epoch_{self.epoch_id}/cam4/cat_all_{count}.png")
        
        print("\n[Validation] All NVS modes tested successfully!")
        return 0.0
