...
class ModelCheckpointCallback_wan2_1_t2v_1_3b_control_normal_depth_train_po_v3(Callback):   
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_t2v_1_3b_control_normal_depth_train_po_v3(Callback):
    ...
    def on_epoch_end(self, trainer):
        if trainer.accelerator.is_main_process:
            trainer.accelerator.log(
                {"val_normal_loss": trainer.val_normal_loss, "val_disparity_loss": trainer.val_disparity_loss},
                step=trainer.global_step,
            )
...
class Trainer_wan2_1_t2v_1_3b_control_normal_depth_train_po_v3:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for data in tqdm(self.val_dataloader, disable=not self.accelerator.is_main_process, desc=f"Epoch {self.epoch_id}: Validating"):
                if data is None:
                    continue
                inference_params["input_videos"] = None
                inference_params["normals"] = None
                inference_params["disparities"] = None
                ...
                inference_params["num_input_videos"] = 2
                # 执行推理
                try:
                    # rgbs process
                    rgbs_input_cond = data["rgbs"].to(self.model.pipe.device).to(self.model.pipe.torch_dtype)[0]
                    
                    # normal process
                    video_dict = self.model.pipe(**inference_params)
                    normal_predict_tensor = video_dict["normal_unit"].to(self.model.pipe.torch_dtype).to(self.model.pipe.device) # torch.tensor[c,t,h,w] [-1,1]
                    normal_gt = data["normals"].to(self.model.pipe.device).to(self.model.pipe.torch_dtype)[0] # c t h w 
                    norm_normal_gt = torch.norm(normal_gt, dim=0, keepdim=True).to(self.model.pipe.torch_dtype).to(self.model.pipe.device)
                    mask = (torch.abs(1 - norm_normal_gt) < 0.5).to(self.model.pipe.torch_dtype).to(self.model.pipe.device) # 1,T,H,W
                    normal_gt_unit = normal_gt / (norm_normal_gt + 1e-6) 
                    loss_normal = (((normal_predict_tensor - normal_gt_unit) * mask) **2).mean()
                    total_normal_loss += loss_normal

                    # disparity process
                    disparity_predict_tensor = video_dict["disparity_loss"].to(self.model.pipe.device).to(self.model.pipe.torch_dtype) # torch.tensor[c,t,h,w] [-1,1]
                    disparity_gt = data["disparities"].to(self.model.pipe.device).to(self.model.pipe.torch_dtype)[0] # c t h w 
                    loss_disparity = (((disparity_predict_tensor - disparity_gt) * mask) **2).mean()
                    total_disparity_loss += loss_disparity
                    ...
                    if self.accelerator.is_main_process:
                        self._tensor2video(rgbs_input_cond * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/rgbs_input_cond_{count}.mp4")
                        self._tensor2video(normal_predict_tensor * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/normal_predict_{count}.mp4")
                        self._tensor2video(normal_gt_unit * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/normal_gt_{count}.mp4")
                        self._tensor2video(torch.cat([rgbs_input_cond * 0.5 + 0.5, normal_predict_tensor * 0.5 + 0.5, normal_gt_unit * 0.5 + 0.5], dim=-1), f"{self.output_path}/epoch_{self.epoch_id}/normal_predict_gt_{count}_{loss_normal}.mp4") # type: ignore

                        disparity_predict_vis = self._colorize_disparity_video(disparity_predict_tensor, mask)
                        disparity_gt_vis = self._colorize_disparity_video(disparity_gt, mask)
                        self._tensor2video(disparity_predict_vis, f"{self.output_path}/epoch_{self.epoch_id}/disparity_predict_{count}.mp4")    
                        self._tensor2video(disparity_gt_vis, f"{self.output_path}/epoch_{self.epoch_id}/disparity_gt_{count}.mp4")
                        self._tensor2video(torch.cat([rgbs_input_cond * 0.5 + 0.5, disparity_predict_vis, disparity_gt_vis], dim=-1), f"{self.output_path}/epoch_{self.epoch_id}/disparity_predict_gt_{count}_{loss_disparity}.mp4") # type: ignore
        ...
        avg_disparity_loss = total_disparity_loss / max(count, 1)
        print(f"[Validation] Epoch {self.epoch_id}: val_normal_loss={avg_normal_loss:.6f}, val_disparity_loss={avg_disparity_loss:.6f}")
        self.model.train()
        return avg_normal_loss, avg_disparity_loss
    ...
    def train(self):
        ...
        for epoch_id in range(self.num_epochs):
            ...
            # 每个epoch结束后做一次验证
            self.val_normal_loss, self.val_disparity_loss = self.validate() # type: ignore
