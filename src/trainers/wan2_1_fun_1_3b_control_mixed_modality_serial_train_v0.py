...
class ModelCheckpointCallback_wan2_1_fun_1_3b_control_mixed_modality_serial_train_v0(Callback):    
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_fun_1_3b_control_mixed_modality_serial_train_v0(Callback):
...
class Trainer_wan2_1_fun_1_3b_control_mixed_modality_serial_train_v0:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for data in tqdm(self.val_dataloader, disable=not self.accelerator.is_main_process, desc=f"Epoch {self.epoch_id}: Validating"):
                ...
                inference_params["control_video"] = data["rgb"].to(self.model.pipe.device).to(self.model.pipe.torch_dtype)
                ...
                try:
                    ...
                    for modality_idx in range(6):
                        data["modality_index"] = modality_idx
                        video_dict = self.model.pipe(data, **inference_params)
                        
                        # 根据模态类型计算损失
                        modality_loss = self._compute_modality_loss(
                            video_dict, data, modality_idx, mask, normal_gt, norm_normal_gt
                        )
                        
                        if modality_loss is not None:
                            total_loss += modality_loss
                            epoch_losses[f"modality_{modality_idx}"] = modality_loss.item()
                        ...
                        self._save_validation_videos(
                            video_dict, data, rgbs_input_cond, epoch_losses, count
                        )
    ...
    def _compute_modality_loss(self, video_dict, data, modality_idx, mask, normal_gt, norm_normal_gt):
        """计算指定模态的损失"""
        try:
            if modality_idx == 0:  # albedo
                if "albedo" not in video_dict:
                    return None
                albedo_predict = video_dict["albedo"].to(self.model.pipe.device)
                albedo_gt = data["albedo"].to(self.model.pipe.device)[0]
                return (((albedo_predict - albedo_gt) * mask) ** 2).mean()
                
            elif modality_idx == 1:  # basecolor
                if "basecolor" not in video_dict:
                    return None
                basecolor_predict = video_dict["basecolor"].to(self.model.pipe.device)
                basecolor_gt = data["basecolor"].to(self.model.pipe.device)[0]
                return (((basecolor_predict - basecolor_gt) * mask) ** 2).mean()
                
            elif modality_idx == 2:  # depth
                if "depth" not in video_dict:
                    return None
                depth_predict = video_dict["depth"].to(self.model.pipe.device)
                depth_gt = data["depth"].to(self.model.pipe.device)[0]
                return (((depth_predict - depth_gt) * mask) ** 2).mean()
                
            elif modality_idx == 3:  # metallic
                if "metallic" not in video_dict:
                    return None
                metallic_predict = video_dict["metallic"].to(self.model.pipe.device)
                metallic_gt = data["metallic"].to(self.model.pipe.device)[0]
                return (((metallic_predict - metallic_gt) * mask) ** 2).mean()
                
            elif modality_idx == 4:  # normal
                if "normal_unit" not in video_dict:
                    return None
                normal_predict = video_dict["normal_unit"].to(self.model.pipe.device)
                normal_gt_unit = normal_gt / (norm_normal_gt + 1e-6)
                return (((normal_predict - normal_gt_unit) * mask) ** 2).mean()
                
            elif modality_idx == 5:  # roughness
                if "roughness" not in video_dict:
                    return None
                roughness_predict = video_dict["roughness"].to(self.model.pipe.device)
                roughness_gt = data["roughness"].to(self.model.pipe.device)[0]
                return (((roughness_predict - roughness_gt) * mask) ** 2).mean()
                
        except Exception as e:
            print(f"计算模态 {modality_idx} 损失时出错: {e}")
            return None
        
        return None

    def _save_validation_videos(self, video_dict, data, rgbs_input_cond, epoch_losses, count):
        """保存验证视频"""
        try:
            modality_idx = video_dict["modality_index"].item()
            modality_names = ["albedo", "basecolor", "depth", "metallic", "normal", "roughness"]
            modality_name = modality_names[modality_idx]
            
            # 获取预测结果和真实值
            if modality_idx == 0:  # albedo
                predict_tensor = video_dict["albedo"]
                gt_tensor = data["albedo"][0]
            elif modality_idx == 1:  # basecolor
                predict_tensor = video_dict["basecolor"]
                gt_tensor = data["basecolor"][0]
            elif modality_idx == 2:  # depth
                predict_tensor = video_dict["depth"]
                gt_tensor = data["depth"][0]
            elif modality_idx == 3:  # metallic
                predict_tensor = video_dict["metallic"]
                gt_tensor = data["metallic"][0]
            elif modality_idx == 4:  # normal
                ...
                predict_tensor = video_dict["roughness"]
                gt_tensor = data["roughness"][0]
            else:
                return
