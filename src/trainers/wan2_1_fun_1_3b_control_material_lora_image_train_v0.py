...
class ModelCheckpointCallback_wan2_1_fun_1_3b_control_material_lora_image_train_v0(Callback):    
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_fun_1_3b_control_material_lora_image_train_v0(Callback):
...
class Trainer_wan2_1_fun_1_3b_control_material_lora_image_train_v0:
    def __init__(
            self, 
            model, 
            optimizer, 
        ...
        with torch.no_grad():
            for data in tqdm(self.val_dataloader, disable=not self.accelerator.is_main_process, desc=f"Epoch {self.epoch_id}: Validating"):
                ...
                try:
                    ...
                    video_dict["modality_index"] = torch.tensor([3], device=self.model.pipe.device)
                    ...
                    if modality_loss is not None:
                        ...
                        epoch_losses[f"modality_{3}"] = modality_loss.item()
