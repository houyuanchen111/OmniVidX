...
class ModelCheckpointCallback_wan2_1_fun_1_3b_control_normal_depth_wonder3d_image_train_v3(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_fun_1_3b_control_normal_depth_wonder3d_image_train_v3(Callback):    
...
class Trainer_wan2_1_fun_1_3b_control_normal_depth_wonder3d_image_train_v3:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for data in tqdm(self.val_dataloader, disable=not self.accelerator.is_main_process, desc=f"Epoch {self.epoch_id}: Validating"):
                ...
                inference_params["control_video"] = torch.cat((data["rgb"], data["rgb"]), dim=0).to(self.model.pipe.device).to(self.model.pipe.torch_dtype)
                ...
                try:
                    ...
                    total_loss += (normal_loss + depth_loss)
