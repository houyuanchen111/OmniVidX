...
class ModelCheckpointCallback_wan2_2_ti2v_5b_normal_depth_train_v0(Callback):    
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_2_ti2v_5b_normal_depth_train_v0(Callback):
...
class Trainer_wan2_2_ti2v_5b_normal_depth_train_v0:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for data in tqdm(self.val_dataloader, disable=not self.accelerator.is_main_process, desc=f"Epoch {self.epoch_id}: Validating"):
                ...
                # inference_params["depths"] = data.get("depths", None)
                ...
                try:
                    ...
                    # disparity process
                        ...
                        depth_predict_tensor = video_dict["depth_loss"].to(self.model.pipe.device).to(self.model.pipe.torch_dtype) # torch.tensor[c,t,h,w] [-1,1]
                        depth_gt = data["depths"].to(self.model.pipe.device).to(self.model.pipe.torch_dtype)[0] # c t h w 
                        loss_depth = (((depth_predict_tensor - depth_gt) * mask) **2).mean()
                    ...
                    if self.accelerator.is_main_process:
                        if video_dict["context_index"].item() == 0:
                            ...
                            depth_predict_vis = self._colorize_disparity_video(depth_predict_tensor, mask)
                            depth_gt_vis = self._colorize_disparity_video(depth_gt, mask)
                            self._tensor2video(depth_predict_vis, f"{self.output_path}/epoch_{self.epoch_id}/depth_predict_{count}.mp4")    
                            self._tensor2video(depth_gt_vis, f"{self.output_path}/epoch_{self.epoch_id}/depth_gt_{count}.mp4")
                            self._tensor2video(torch.cat([rgbs_input_cond * 0.5 + 0.5, depth_predict_vis, depth_gt_vis], dim=-1), f"{self.output_path}/epoch_{self.epoch_id}/depth_predict_gt_{count}_{loss_depth}.mp4") # type: ignore
