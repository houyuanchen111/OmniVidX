...
class ModelCheckpointCallback_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_lora_image_prism_train_v1(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_lora_image_prism_train_v1(Callback):    
...
class Trainer_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_lora_image_prism_train_v1:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        # 提取数据
        inference_rgb = val_batch["im"]
        inference_normal = val_batch["normal"]
        inference_albedo = val_batch["albedo"]
        self.model.eval()
        total_loss = 0
        count = 0
        # RGBX_Generation
        inference_params = {
            ...
            "prompt":"一位身穿淡雅汉服的古典女子，正襟危坐于一张红木古筝前，纤细的手指轻抚琴弦。她神情专注而宁静，长发用一根简单的玉簪挽起。",
            ...
            "training_mode": "RGBX_Generation",
        }   

        video_dict = self.model.pipe(**inference_params)
        os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/RGBX_Generation", exist_ok=True)
        # 保存各个模态的生成视频
        rgb_gen = video_dict["rgb"][:,0]
        torchvision.utils.save_image(rgb_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBX_Generation/rgb_gen_{count}.png")
        albedo_gen = video_dict["albedo"][:,0]
        torchvision.utils.save_image(albedo_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBX_Generation/albedo_gen_{count}.png")
        irradiance_gen = video_dict["irradiance"][:,0]
        torchvision.utils.save_image(irradiance_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBX_Generation/irradiance_gen_{count}.png")
        normal_gen = video_dict["normal_unit"][:,0]
        torchvision.utils.save_image(normal_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBX_Generation/normal_gen_{count}.png")
        ...
        modalities_to_cat = [
            rgb_gen * 0.5 + 0.5,
            albedo_gen * 0.5 + 0.5,
            irradiance_gen * 0.5 + 0.5,
            normal_gen * 0.5 + 0.5
        ]
        cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接 [c, t, h, w*4]
        torchvision.utils.save_image(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/RGBX_Generation/cat_all_modalities_gen_{count}.png")
        ...
        os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/RGB2X", exist_ok=True)
        albedo_gen = video_dict["albedo"][:,0]
        torchvision.utils.save_image(albedo_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGB2X/albedo_gen_{count}.png")
        irradiance_gen = video_dict["irradiance"][:,0]
        torchvision.utils.save_image(irradiance_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGB2X/irradiance_gen_{count}.png")
        normal_gen = video_dict["normal_unit"][:,0]
        torchvision.utils.save_image(normal_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGB2X/normal_gen_{count}.png")

        
        modalities_to_cat = [
            inference_rgb[0,:,0] * 0.5 + 0.5,
            albedo_gen * 0.5 + 0.5,
            irradiance_gen * 0.5 + 0.5,
            normal_gen * 0.5 + 0.5
        ]
        cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接 [c, t, h, w*5]
        torchvision.utils.save_image(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/RGB2X/cat_all_modalities_gen_{count}.png")
        # Normal2RGBX
        inference_params = {
            # "prompt":"纪实摄影风格画面，一只活泼的小狗在绿茵茵的草地上迅速奔跑。小狗毛色棕黄，两只耳朵立起，神情专注而欢快。阳光洒在它身上，使得毛发看上去格外柔软而闪亮。背景是一片开阔的草地，偶尔点缀着几朵野花，远处隐约可见蓝天和几片白云。透视感鲜明，捕捉小狗奔跑时的动感和四周草地的生机。中景侧面移动视角。",
        ...
        os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/Normal2RGBX", exist_ok=True)
        rgb_gen = video_dict["rgb"][:,0]
        torchvision.utils.save_image(rgb_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Normal2RGBX/rgb_gen_{count}.png")
        albedo_gen = video_dict["albedo"][:,0]
        torchvision.utils.save_image(albedo_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Normal2RGBX/albedo_gen_{count}.png")
        irradiance_gen = video_dict["irradiance"][:,0]
        torchvision.utils.save_image(irradiance_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Normal2RGBX/irradiance_gen_{count}.png")
        ...
        modalities_to_cat = [
            ...
            albedo_gen * 0.5 + 0.5,
            irradiance_gen * 0.5 + 0.5,
            inference_normal[0,:,0] * 0.5 + 0.5
        ]
        cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接 [c, t, h, w*5]
        torchvision.utils.save_image(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Normal2RGBX/cat_all_modalities_gen_{count}.png")
        # Relight
        inference_params = {
            # "prompt":"纪实摄影风格画面，一只活泼的小狗在绿茵茵的草地上迅速奔跑。小狗毛色棕黄，两只耳朵立起，神情专注而欢快。阳光洒在它身上，使得毛发看上去格外柔软而闪亮。背景是一片开阔的草地，偶尔点缀着几朵野花，远处隐约可见蓝天和几片白云。透视感鲜明，捕捉小狗奔跑时的动感和四周草地的生机。中景侧面移动视角。",
            "prompt": "赛博朋克霓虹灯光，强烈的蓝紫色和粉红色霓虹灯的场景。",
            "negative_prompt":"色调艳丽，过曝，静态，细节模糊不清，字幕，风格，作品，画作，画面，静止，整体发灰，最差质量，低质量，JPEG压缩残留，丑陋的，残缺的，多余的手指，画得不好的手部，画得不好的脸部，畸形的，毁容的，形态畸形的肢体，手指融合，静止不动的画面，杂乱的背景，三条腿，背景人很多，倒着走",
            "seed": 1,
            "num_inference_steps": 50,
            "cfg_scale": 5.0,
        ...
        # 保存各个模态的生成视频
        os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/Relight", exist_ok=True)
        rgb_gen = video_dict["rgb"][:,0]
        torchvision.utils.save_image(rgb_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Relight/rgb_gen_{count}.png")
        irradiance_gen = video_dict["irradiance"][:,0]
        torchvision.utils.save_image(irradiance_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Relight/irradiance_gen_{count}.png")
        ...
        modalities_to_cat = [
            rgb_gen * 0.5 + 0.5,
            inference_albedo[0,:,0] * 0.5 + 0.5,
            irradiance_gen * 0.5 + 0.5,
            inference_normal[0,:,0] * 0.5 + 0.5
        ]
        cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接 [c, t, h, w*5]
        torchvision.utils.save_image(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Relight/cat_all_modalities_gen_{count}.png")
        return 0.0
