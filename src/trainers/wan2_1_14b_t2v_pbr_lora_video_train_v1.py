...
class ModelCheckpointCallback_wan2_1_14b_t2v_pbr_lora_video_train_v1(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_14b_t2v_pbr_lora_video_train_v1(Callback):    
...
class Trainer_wan2_1_14b_t2v_pbr_lora_video_train_v1:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        # ========== 1. PBRGeneration: 全生成模式 ==========
        print("\n[Validation] Testing PBRGeneration mode...")
        inference_params = {
            ...
            "training_mode": "PBRGeneration",
        }   

        video_dict = self.model.pipe(**inference_params)
        os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/PBRGeneration", exist_ok=True)
        
        # 保存各个分支的生成视频
        if "rgb" in video_dict:
            rgb_gen = video_dict["rgb"]  # [C, T, H, W]
            self._tensor2video(rgb_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/PBRGeneration/rgb_gen_{count}.mp4")
        if "albedo" in video_dict:
            albedo_gen = video_dict["albedo"]
            self._tensor2video(albedo_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/PBRGeneration/albedo_gen_{count}.mp4")
        if "irradiance" in video_dict:
            irradiance_gen = video_dict["irradiance"]
            self._tensor2video(irradiance_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/PBRGeneration/irradiance_gen_{count}.mp4")
        if "normal_unit" in video_dict:
            normal_gen = video_dict["normal_unit"]
            self._tensor2video(normal_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/PBRGeneration/normal_gen_{count}.mp4")
        ...
        if modalities_to_cat:
            ...
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/PBRGeneration/cat_all_{count}.mp4")
 
        # ========== 2. Inverse_Rendering: 抠图任务 (com -> pha, fgr, bgr) ==========
        print("\n[Validation] Testing Inverse_Rendering mode...")
        ...
        if inference_rgb is not None:
            inference_params = {
                ...
                "inference_irradiance": None,
                "inference_normal": None,
                "training_mode": "Inverse_Rendering",
            }   

            video_dict = self.model.pipe(**inference_params)
            os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/Inverse_Rendering", exist_ok=True)
            
            # 保存预测视频
            if "albedo" in video_dict:
                albedo_gen = video_dict["albedo"]
                self._tensor2video(albedo_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Inverse_Rendering/albedo_gen_{count}.mp4")
            if "irradiance" in video_dict:
                irradiance_gen = video_dict["irradiance"]
                self._tensor2video(irradiance_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Inverse_Rendering/irradiance_gen_{count}.mp4")
            if "normal_unit" in video_dict:
                normal_gen = video_dict["normal_unit"]
                self._tensor2video(normal_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Inverse_Rendering/normal_gen_{count}.mp4")
            ...
            cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Inverse_Rendering/cat_all_{count}.mp4")
        # ========== 3. Relighting: ==========
        print("\n[Validation] Testing Relighting mode...")
        if inference_albedo is not None and inference_normal is not None:
            ...
            video_dict = self.model.pipe(**inference_params)
            os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/Relighting", exist_ok=True)
            
            # 保存预测视频
            if "rgb" in video_dict:
                rgb_gen = video_dict["rgb"]
                self._tensor2video(rgb_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Relighting/rgb_gen_{count}.mp4")
            if "irradiance" in video_dict:
                irradiance_gen = video_dict["irradiance"]
                self._tensor2video(irradiance_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Relighting/irradiance_gen_{count}.mp4")
            ...
            cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Relighting/cat_all_{count}.mp4")
        ...
        if inference_rgb is not None:
            ...
            video_dict = self.model.pipe(**inference_params)
            os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/Reconstruction", exist_ok=True)
            
            # 保存预测视频
            if "rgb" in video_dict:
                rgb_gen = video_dict["rgb"]
                self._tensor2video(rgb_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Reconstruction/rgb_gen_{count}.mp4")
            ...
            cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Reconstruction/cat_all_{count}.mp4")
