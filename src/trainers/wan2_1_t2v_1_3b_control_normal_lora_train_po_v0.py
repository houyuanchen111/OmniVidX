...
class ModelCheckpointCallback_wan2_1_t2v_1_3b_control_normal_lora_train_po_v0(Callback):
    """
    checkpoint callback
    """
    ...
    def on_epoch_end(self, trainer):
        ...
        if trainer.accelerator.is_main_process:
            ...
            print(f"Epoch {trainer.epoch_id}: 正在保存resume权重...")
            self._save_resume_weights(trainer, epoch_checkpoint_dir)

class TensorboardLoggingCallback_wan2_1_t2v_1_3b_control_normal_lora_train_po_v0(Callback):
...
class Trainer_wan2_1_t2v_1_3b_control_normal_lora_train_po_v0:
    def __init__(
            self, 
            model, 
            optimizer, 
        ...
        with torch.no_grad():
            for data in tqdm(self.val_dataloader, disable=not self.accelerator.is_main_process, desc=f"Epoch {self.epoch_id}: Validating"):
                ...
                try:
                    ...
                    self._tensor2video(torch.cat([normal_predict_tensor * 0.5 + 0.5, normal_gt_unit * 0.5 + 0.5], dim=-1), f"{self.output_path}/epoch_{self.epoch_id}/normal_predict_gt_{count}_{loss_normal}.mp4")
                    count += 1
