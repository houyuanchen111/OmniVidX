...
class ModelCheckpointCallback_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_lora_video_train_v4(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_lora_video_train_v4(Callback):    
...
class Trainer_wan2_1_14b_t2v_pbrgen_pbr_sft_rec_gen_lora_video_train_v4:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        total_loss = 0
        count = 0
        inference_image = torch.from_numpy(np.transpose(np.stack([frame for frame in imageio.get_reader("debug_480_832.mp4")]), (3, 0, 1, 2))).float() / 255.0
        inference_image = inference_image[None][:,:,:21] *2 -1
        # rec的validate
    ...
    def train(self):
        ...
        for epoch_id in range(self.num_epochs):
            ...
            if self.num_epochs == 100:
                ...
                if self.epoch_id % 99 == 0 and self.epoch_id != 0: # 每99个epoch做一次验证
                    if self.accelerator.is_main_process:
                        try:
                            # self.val_loss = self.validate() # type: ignore 直接不validate
                            self.val_loss = 0.0
                        except Exception as e:
                            print(f"❌ 验证推理失败: {e}")
                            import traceback
