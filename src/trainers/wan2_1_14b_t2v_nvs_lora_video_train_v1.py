...
class ModelCheckpointCallback_wan2_1_14b_t2v_nvs_lora_video_train_v1(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_14b_t2v_nvs_lora_video_train_v1(Callback):    
...
class Trainer_wan2_1_14b_t2v_nvs_lora_video_train_v1:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        if "cam1" in video_dict:
            cam1_gen = video_dict["cam1"]  # [C, T, H, W]
            self._tensor2video(cam1_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Generation/cam1_gen_{count}.mp4")
        if "cam2" in video_dict:
            cam2_gen = video_dict["cam2"]
            self._tensor2video(cam2_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Generation/cam2_gen_{count}.mp4")
        if "cam3" in video_dict:
            cam3_gen = video_dict["cam3"]
            self._tensor2video(cam3_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Generation/cam3_gen_{count}.mp4")
        if "cam4" in video_dict:
            cam4_gen = video_dict["cam4"]
            self._tensor2video(cam4_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Generation/cam4_gen_{count}.mp4")

        # 保存拼接视频（横向拼接所有相机）
        cameras_to_cat = []
        if "cam1" in video_dict:
            cameras_to_cat.append(video_dict["cam1"] * 0.5 + 0.5)
        if "cam2" in video_dict:
            cameras_to_cat.append(video_dict["cam2"] * 0.5 + 0.5)
        if "cam3" in video_dict:
            cameras_to_cat.append(video_dict["cam3"] * 0.5 + 0.5)
        if "cam4" in video_dict:
            cameras_to_cat.append(video_dict["cam4"] * 0.5 + 0.5)
        
        if cameras_to_cat:
            cat_video = torch.cat(cameras_to_cat, dim=-1)  # 在宽度维度拼接
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Generation/cat_all_{count}.mp4")
 
        # ========== 2. cam1: 以cam1为条件，预测cam2, cam3, cam4 ==========
        ...
        if inference_cam1 is not None:
            ...
            if "cam2" in video_dict:
                cam2_gen = video_dict["cam2"]
                self._tensor2video(cam2_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam1/cam2_gen_{count}.mp4")
            if "cam3" in video_dict:
                cam3_gen = video_dict["cam3"]
                self._tensor2video(cam3_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam1/cam3_gen_{count}.mp4")
            if "cam4" in video_dict:
                cam4_gen = video_dict["cam4"]
                self._tensor2video(cam4_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam1/cam4_gen_{count}.mp4")

            # 保存拼接视频 (输入cam1 + 预测的cam2, cam3, cam4)
            cameras_to_cat = [inference_cam1[0] * 0.5 + 0.5]  # [C, T, H, W]
            if "cam2" in video_dict:
                cameras_to_cat.append(video_dict["cam2"] * 0.5 + 0.5)
            if "cam3" in video_dict:
                cameras_to_cat.append(video_dict["cam3"] * 0.5 + 0.5)
            ...
            cat_video = torch.cat(cameras_to_cat, dim=-1)  # 在宽度维度拼接
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/cam1/cat_all_{count}.mp4")
            
        # ========== 3. cam2: 以cam2为条件，预测cam1, cam3, cam4 ==========
        ...
        if inference_cam2 is not None:
            ...
            # 保存预测视频
            if "cam1" in video_dict:
                cam1_gen = video_dict["cam1"]
                self._tensor2video(cam1_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam2/cam1_gen_{count}.mp4")
            if "cam3" in video_dict:
                cam3_gen = video_dict["cam3"]
                self._tensor2video(cam3_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam2/cam3_gen_{count}.mp4")
            if "cam4" in video_dict:
                cam4_gen = video_dict["cam4"]
                self._tensor2video(cam4_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam2/cam4_gen_{count}.mp4")

            # 保存拼接视频
            cameras_to_cat = []
            if "cam1" in video_dict:
                cameras_to_cat.append(video_dict["cam1"] * 0.5 + 0.5)
            cameras_to_cat.append(inference_cam2[0] * 0.5 + 0.5)
            if "cam3" in video_dict:
                cameras_to_cat.append(video_dict["cam3"] * 0.5 + 0.5)
            if "cam4" in video_dict:
                cameras_to_cat.append(video_dict["cam4"] * 0.5 + 0.5)
            
            cat_video = torch.cat(cameras_to_cat, dim=-1)
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/cam2/cat_all_{count}.mp4")
            
        # ========== 4. cam3: 以cam3为条件，预测cam1, cam2, cam4 ==========
        ...
        if inference_cam3 is not None:
            ...
            if "cam1" in video_dict:
                cam1_gen = video_dict["cam1"]
                self._tensor2video(cam1_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam3/cam1_gen_{count}.mp4")
            if "cam2" in video_dict:
                cam2_gen = video_dict["cam2"]
                self._tensor2video(cam2_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam3/cam2_gen_{count}.mp4")
            if "cam4" in video_dict:
                cam4_gen = video_dict["cam4"]
                self._tensor2video(cam4_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam3/cam4_gen_{count}.mp4")
            ...
            if "cam2" in video_dict:
                cameras_to_cat.append(video_dict["cam2"] * 0.5 + 0.5)
            cameras_to_cat.append(inference_cam3[0] * 0.5 + 0.5)
            if "cam4" in video_dict:
                cameras_to_cat.append(video_dict["cam4"] * 0.5 + 0.5)
            
            cat_video = torch.cat(cameras_to_cat, dim=-1)
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/cam3/cat_all_{count}.mp4")
            
        # ========== 5. cam4: 以cam4为条件，预测cam1, cam2, cam3 ==========
        ...
        if inference_cam4 is not None:
            ...
            if "cam1" in video_dict:
                cam1_gen = video_dict["cam1"]
                self._tensor2video(cam1_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam4/cam1_gen_{count}.mp4")
            if "cam2" in video_dict:
                cam2_gen = video_dict["cam2"]
                self._tensor2video(cam2_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam4/cam2_gen_{count}.mp4")
            if "cam3" in video_dict:
                cam3_gen = video_dict["cam3"]
                self._tensor2video(cam3_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/cam4/cam3_gen_{count}.mp4")
            ...
            if "cam3" in video_dict:
                cameras_to_cat.append(video_dict["cam3"] * 0.5 + 0.5)
            cameras_to_cat.append(inference_cam4[0] * 0.5 + 0.5)
            
            cat_video = torch.cat(cameras_to_cat, dim=-1)
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/cam4/cat_all_{count}.mp4")
