"""
这边是wan2.1-fun-1.3b-control-normal-train的trainer
可训练self.attn
"""
...
class ModelCheckpointCallback_wan2_1_t2v_1_3b_control_pointmap_train_po_v0(Callback):
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_t2v_1_3b_control_pointmap_train_po_v0(Callback):
...
class Trainer_wan2_1_t2v_1_3b_control_pointmap_train_po_v0:
    def __init__(
            self, 
            model, 
            optimizer, 
            ...
            for data in tqdm(self.val_dataloader, disable=not self.accelerator.is_main_process, desc=f"Epoch {self.epoch_id}: Validating"):
                ...
                try:
                    ...
                    if self.accelerator.is_main_process:
                        ...
                        self._tensor2video(torch.cat([rgbs_input_cond * 0.5 + 0.5, normal_predict_tensor * 0.5 + 0.5, normal_gt_unit * 0.5 + 0.5], dim=-1), f"{self.output_path}/epoch_{self.epoch_id}/normal_predict_gt_{count}_{loss_normal}.mp4")
                    count += 1
