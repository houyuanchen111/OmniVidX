...
class ModelCheckpointCallback_wan2_1_14b_t2v_matte_lora_video_train_v9(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_14b_t2v_matte_lora_video_train_v9(Callback):    
...
class Trainer_wan2_1_14b_t2v_matte_lora_video_train_v9:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        print("\n[Validation] Testing t2RPFB mode...")
        inference_params = {
            ...
            "inference_fgr": None,
            "inference_bgr": None,
            "training_mode": "t2RPFB",
        }   

        video_dict = self.model.pipe(**inference_params)
        os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/t2RPFB", exist_ok=True)
        
        # 保存各个分支的生成视频
        if "com" in video_dict:
            com_gen = video_dict["com"]  # [C, T, H, W]
            self._tensor2video(com_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/t2RPFB/com_gen_{count}.mp4")
        if "pha" in video_dict:
            pha_gen = video_dict["pha"]
            self._tensor2video(pha_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/t2RPFB/pha_gen_{count}.mp4")
        if "fgr" in video_dict:
            fgr_gen = video_dict["fgr"]
            self._tensor2video(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/t2RPFB/fgr_gen_{count}.mp4")
        if "bgr" in video_dict:
            bgr_gen = video_dict["bgr"]
            self._tensor2video(bgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/t2RPFB/bgr_gen_{count}.mp4")
        ...
        if modalities_to_cat:
            ...
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/t2RPFB/cat_all_{count}.mp4")
 
        # ========== 2. Matte: 抠图任务 (com -> pha, fgr, bgr) ==========
        print("\n[Validation] Testing R2PFB mode...")
        if inference_rgb is not None:
            ...
            video_dict = self.model.pipe(**inference_params)
            os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/R2PFB", exist_ok=True)
            
            # 保存预测视频
            if "pha" in video_dict:
                pha_gen = video_dict["pha"]
                self._tensor2video(pha_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/R2PFB/pha_gen_{count}.mp4")
            if "fgr" in video_dict:
                fgr_gen = video_dict["fgr"]
                self._tensor2video(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/R2PFB/fgr_gen_{count}.mp4")
            if "bgr" in video_dict:
                bgr_gen = video_dict["bgr"]
                self._tensor2video(bgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/R2PFB/bgr_gen_{count}.mp4")

            # 保存拼接视频 (输入com + 预测的pha, fgr, bgr)
            modalities_to_cat = [inference_rgb[0] * 0.5 + 0.5]  # [C, T, H, W]
            if "pha" in video_dict:
                modalities_to_cat.append(video_dict["pha"] * 0.5 + 0.5)
            if "fgr" in video_dict:
                modalities_to_cat.append(video_dict["fgr"] * 0.5 + 0.5)
            ...
            cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/R2PFB/cat_all_{count}.mp4")
        ...
        #     modalities_to_cat = []
        #     if inference_rgb is not None:
        #         modalities_to_cat.append(inference_rgb[0] * 0.5 + 0.5)
        #     modalities_to_cat.append(inference_pha[0] * 0.5 + 0.5)
        #     if "fgr" in video_dict:
        #         modalities_to_cat.append(video_dict["fgr"] * 0.5 + 0.5)
        ...
        print("\n[Validation] Testing B2RPF mode...")
        if inference_rgb is not None and inference_pha is not None and inference_fgr is not None:
            ...
            if inference_bgr is not None:
                ...
                video_dict = self.model.pipe(**inference_params)
                os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/B2RPF", exist_ok=True)

                # 保存预测 com/pha/fgr
                if "rgb" in video_dict:
                    rgb_gen = video_dict["rgb"]
                    self._tensor2video(rgb_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/B2RPF/rgb_gen_{count}.mp4")
                if "pha" in video_dict:
                    pha_gen = video_dict["pha"]
                    self._tensor2video(pha_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/B2RPF/pha_gen_{count}.mp4")
                if "fgr" in video_dict:
                    fgr_gen = video_dict["fgr"]
                    self._tensor2video(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/B2RPF/fgr_gen_{count}.mp4")
                ...
                modalities_to_cat.append(inference_bgr[0] * 0.5 + 0.5)
                cat_video = torch.cat(modalities_to_cat, dim=-1)
                self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/B2RPF/cat_all_{count}.mp4")
