...
class ModelCheckpointCallback_wan2_1_14b_t2v_matte_lora_video_train_v1(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_14b_t2v_matte_lora_video_train_v1(Callback):    
...
class Trainer_wan2_1_14b_t2v_matte_lora_video_train_v1:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        # ========== 3. Inpainting: 修复任务 (pha, bgr -> com, fgr) ==========
        print("\n[Validation] Testing Inpainting mode...")
        if inference_pha is not None and inference_bgr is not None:
            inference_params = {
            ...
            if "com" in video_dict:
                ...
                self._tensor2video(com_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Inpainting/com_gen_{count}.mp4")
            if "fgr" in video_dict:
                fgr_gen = video_dict["fgr"]
                self._tensor2video(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Inpainting/fgr_gen_{count}.mp4")

            # 保存拼接视频 (输入pha, bgr + 预测的com, fgr)
            modalities_to_cat = []
            if "com" in video_dict:
                modalities_to_cat.append(video_dict["com"] * 0.5 + 0.5)
            ...
            cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Inpainting/cat_all_{count}.mp4")
