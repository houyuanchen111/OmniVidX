...
class ModelCheckpointCallback_wan2_1_14b_t2v_matte_lora_image_train_v1(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_14b_t2v_matte_lora_image_train_v1(Callback):    
...
class Trainer_wan2_1_14b_t2v_matte_lora_image_train_v1:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        inference_params = {
            ...
            "prompt":"一位身穿淡雅汉服的古典女子在眺望远方。背景为：古代繁华的洛阳城内。",
        ...
        if "com" in video_dict:
            com_gen = video_dict["com"][:,0]
            torchvision.utils.save_image(com_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration/com_gen_{count}.png")
        if "pha" in video_dict:
            pha_gen = video_dict["pha"][:,0]
            torchvision.utils.save_image(pha_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration/pha_gen_{count}.png")
        if "fgr" in video_dict:
            fgr_gen = video_dict["fgr"][:,0]
            torchvision.utils.save_image(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration/fgr_gen_{count}.png")
        if "bgr" in video_dict:
            bgr_gen = video_dict["bgr"][:,0]
            torchvision.utils.save_image(bgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration/bgr_gen_{count}.png")

        # 保存拼接图像
        modalities_to_cat = []
        if "com" in video_dict:
            modalities_to_cat.append(video_dict["com"][:,0] * 0.5 + 0.5)
        if "pha" in video_dict:
            modalities_to_cat.append(video_dict["pha"][:,0] * 0.5 + 0.5)
        if "fgr" in video_dict:
            modalities_to_cat.append(video_dict["fgr"][:,0] * 0.5 + 0.5)
        if "bgr" in video_dict:
            modalities_to_cat.append(video_dict["bgr"][:,0] * 0.5 + 0.5)
        
        if modalities_to_cat:
            cat_video = torch.cat(modalities_to_cat, dim=-1)
            torchvision.utils.save_image(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration/cat_all_{count}.png")
 
        # ========== 2. Matte: 抠图任务 (com -> pha, fgr, bgr) ==========
        print("\n[Validation] Testing Matte mode...")
        if inference_com is not None:
            inference_params = {
                "prompt": "",  # Matte 任务不需要 prompt
                "negative_prompt":"色调艳丽，过曝，静态，细节模糊不清，字幕，风格，作品，画作，画面，静止，整体发灰，最差质量，低质量，JPEG压缩残留，丑陋的，残缺的，多余的手指，画得不好的手部，画得不好的脸部，畸形的，毁容的，形态畸形的肢体，手指融合，静止不动的画面，杂乱的背景，三条腿，背景人很多，倒着走",
                "seed": 1,
                "num_inference_steps": 50,
            ...
            if "pha" in video_dict:
                pha_gen = video_dict["pha"][:,0]
                torchvision.utils.save_image(pha_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Matte/pha_gen_{count}.png")
            if "fgr" in video_dict:
                fgr_gen = video_dict["fgr"][:,0]
                torchvision.utils.save_image(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Matte/fgr_gen_{count}.png")
            if "bgr" in video_dict:
                bgr_gen = video_dict["bgr"][:,0]
                torchvision.utils.save_image(bgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Matte/bgr_gen_{count}.png")

            # 保存拼接图像 (输入com + 预测的pha, fgr, bgr)
            modalities_to_cat = [inference_com[0,:,0] * 0.5 + 0.5]
            if "pha" in video_dict:
                modalities_to_cat.append(video_dict["pha"][:,0] * 0.5 + 0.5)
            if "fgr" in video_dict:
                modalities_to_cat.append(video_dict["fgr"][:,0] * 0.5 + 0.5)
            ...
            cat_video = torch.cat(modalities_to_cat, dim=-1)
            torchvision.utils.save_image(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Matte/cat_all_{count}.png")
        # ========== 3. Inpainting: 修复任务 (pha, fgr -> com, fgr) ==========
        print("\n[Validation] Testing Inpainting mode...")
        if inference_pha is not None and inference_fgr is not None:
            inference_params = {
                "prompt": val_batch.get("prompt", [""])[0],
                "negative_prompt":"色调艳丽，过曝，静态，细节模糊不清，字幕，风格，作品，画作，画面，静止，整体发灰，最差质量，低质量，JPEG压缩残留，丑陋的，残缺的，多余的手指，画得不好的手部，画得不好的脸部，畸形的，毁容的，形态畸形的肢体，手指融合，静止不动的画面，杂乱的背景，三条腿，背景人很多，倒着走",
                "seed": 1,
                "num_inference_steps": 50,
                "cfg_scale": 3.0,
            ...
            video_dict = self.model.pipe(**inference_params)
            os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/Inpainting", exist_ok=True)
            
            # 保存预测结果
            if "com" in video_dict:
                com_gen = video_dict["com"][:,0]
                torchvision.utils.save_image(com_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Inpainting/com_gen_{count}.png")
            if "fgr" in video_dict:
                fgr_gen = video_dict["fgr"][:,0]
                torchvision.utils.save_image(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Inpainting/fgr_gen_{count}.png")

            # 保存拼接图像 (输入pha, fgr + 预测的com, fgr)
            modalities_to_cat = []
            if "com" in video_dict:
                modalities_to_cat.append(video_dict["com"][:,0] * 0.5 + 0.5)
            modalities_to_cat.append(inference_pha[0,:,0] * 0.5 + 0.5)
            if "fgr" in video_dict:
                modalities_to_cat.append(video_dict["fgr"][:,0] * 0.5 + 0.5)
            modalities_to_cat.append(inference_bgr[0,:,0] * 0.5 + 0.5)  
            
            cat_video = torch.cat(modalities_to_cat, dim=-1)
            torchvision.utils.save_image(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Inpainting/cat_all_{count}.png")
        # ========== 4. BackgroundGeneration: 背景生成 (pha, fgr -> com, bgr) ==========
        print("\n[Validation] Testing BackgroundGeneration mode...")
        if inference_pha is not None and inference_fgr is not None:
            inference_params = {
                "prompt": val_batch.get("fgr_prompt", [""])[0] + "背景为:东京繁华的街头。", 
                "negative_prompt": "模糊，低质量，变形，畸形",
                "seed": 1,
                "num_inference_steps": 50,
                "cfg_scale": 5.0,
                "cfg_merge": False,
            ...
            if "com" in video_dict:
                com_gen = video_dict["com"][:,0]
                torchvision.utils.save_image(com_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/BackgroundGeneration/com_gen_{count}.png")
            if "bgr" in video_dict:
                bgr_gen = video_dict["bgr"][:,0]
                torchvision.utils.save_image(bgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/BackgroundGeneration/bgr_gen_{count}.png")

            # 保存拼接图像 (输入pha, fgr + 预测的com, bgr)
            modalities_to_cat = []
            if "com" in video_dict:
                modalities_to_cat.append(video_dict["com"][:,0] * 0.5 + 0.5)
            modalities_to_cat.append(inference_pha[0,:,0] * 0.5 + 0.5)
            modalities_to_cat.append(inference_fgr[0,:,0] * 0.5 + 0.5)
            if "bgr" in video_dict:
                modalities_to_cat.append(video_dict["bgr"][:,0] * 0.5 + 0.5)
            
            cat_video = torch.cat(modalities_to_cat, dim=-1)
            torchvision.utils.save_image(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/BackgroundGeneration/cat_all_{count}.png")
