...
class ModelCheckpointCallback_wan2_1_14b_t2v_matte_lora_video_train_v7(Callback):     
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_14b_t2v_matte_lora_video_train_v7(Callback):    
...
class Trainer_wan2_1_14b_t2v_matte_lora_video_train_v7:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        inference_com = val_batch.get("com")
        inference_pha = val_batch.get("pha")
        inference_fgr = val_batch.get("fgr")
        inference_bgr = val_batch.get("bgr")
        ...
        # return 0.0
        # ========== 1. RGBAGeneration: 全生成模式 ==========
        print("\n[Validation] Testing RGBAGeneration mode...")
        inference_params = {
            # "prompt":"纪实摄影风格画面，一只活泼的小狗在绿茵茵的草地上迅速奔跑。小狗毛色棕黄，两只耳朵立起，神情专注而欢快。阳光洒在它身上，使得毛发看上去格外柔软而闪亮。背景是一片开阔的草地，偶尔点缀着几朵野花，远处隐约可见蓝天和几片白云。透视感鲜明，捕捉小狗奔跑时的动感和四周草地的生机。中景侧面移动视角。",
            "prompt": [
            "一位身穿淡雅汉服的古典女子在眺望远方。背景为：古代繁华的洛阳城内。", 
            "一位身穿淡雅汉服的古典女子在眺望远方。背景为：古代繁华的洛阳城内。", 
            "一位身穿淡雅汉服的古典女子在眺望远方。背景为：古代繁华的洛阳城内。",
            "一位身穿淡雅汉服的古典女子在眺望远方。背景为：古代繁华的洛阳城内。", 
            ],
            ...
            "inference_fgr": None,
            "inference_bgr": None,
            "training_mode": "RGBAGeneration",
        }   

        video_dict = self.model.pipe(**inference_params)
        os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration", exist_ok=True)
        
        # 保存各个分支的生成视频
        if "com" in video_dict:
            com_gen = video_dict["com"]  # [C, T, H, W]
            self._tensor2video(com_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration/com_gen_{count}.mp4")
        if "pha" in video_dict:
            pha_gen = video_dict["pha"]
            self._tensor2video(pha_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration/pha_gen_{count}.mp4")
        if "fgr" in video_dict:
            fgr_gen = video_dict["fgr"]
            self._tensor2video(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration/fgr_gen_{count}.mp4")
        if "bgr" in video_dict:
            bgr_gen = video_dict["bgr"]
            self._tensor2video(bgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration/bgr_gen_{count}.mp4")

        # 保存拼接视频（横向拼接所有模态）
        modalities_to_cat = []
        if "com" in video_dict:
            modalities_to_cat.append(video_dict["com"] * 0.5 + 0.5)
        if "pha" in video_dict:
            modalities_to_cat.append(video_dict["pha"] * 0.5 + 0.5)
        if "fgr" in video_dict:
            modalities_to_cat.append(video_dict["fgr"] * 0.5 + 0.5)
        if "bgr" in video_dict:
            modalities_to_cat.append(video_dict["bgr"] * 0.5 + 0.5)
        
        if modalities_to_cat:
            cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/RGBAGeneration/cat_all_{count}.mp4")
 
        # ========== 2. Matte: 抠图任务 (com -> pha, fgr, bgr) ==========
        print("\n[Validation] Testing Matte mode...")
        if inference_com is not None:
            ...
            video_dict = self.model.pipe(**inference_params)
            os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/Matte", exist_ok=True)
            
            # 保存预测视频
            if "pha" in video_dict:
                pha_gen = video_dict["pha"]
                self._tensor2video(pha_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Matte/pha_gen_{count}.mp4")
            if "fgr" in video_dict:
                fgr_gen = video_dict["fgr"]
                self._tensor2video(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Matte/fgr_gen_{count}.mp4")
            if "bgr" in video_dict:
                bgr_gen = video_dict["bgr"]
                self._tensor2video(bgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Matte/bgr_gen_{count}.mp4")

            # 保存拼接视频 (输入com + 预测的pha, fgr, bgr)
            modalities_to_cat = [inference_com[0] * 0.5 + 0.5]  # [C, T, H, W]
            if "pha" in video_dict:
                modalities_to_cat.append(video_dict["pha"] * 0.5 + 0.5)
            if "fgr" in video_dict:
                modalities_to_cat.append(video_dict["fgr"] * 0.5 + 0.5)
            ...
            cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接
            self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Matte/cat_all_{count}.mp4")
        # ========== 3. BackgroundGeneration: 背景生成 (pha, fgr -> com, bgr) ==========
        # print("\n[Validation] Testing BackgroundGeneration mode...")
        # if inference_pha is not None and inference_fgr is not None:
        #     inference_params = {
        #         "prompt": 
        #         [
        #             "东京繁华的街头。",
        ...
        #         "tile_stride": [15, 26],
        #         "is_inference": True,
        #         "training_mode": "BackgroundGeneration",
        #         "inference_com": None,
        #         "inference_pha": inference_pha,
        #         "inference_fgr": inference_fgr,
        #         "inference_bgr": None,
        #     }   

        #     video_dict = self.model.pipe(**inference_params)
        #     os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/BackgroundGeneration", exist_ok=True)
            
        #     # 保存预测视频
        #     if "com" in video_dict:
        #         com_gen = video_dict["com"]
        #         self._tensor2video(com_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/BackgroundGeneration/com_gen_{count}.mp4")
        #     if "bgr" in video_dict:
        #         bgr_gen = video_dict["bgr"]
        #         self._tensor2video(bgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/BackgroundGeneration/bgr_gen_{count}.mp4")

        #     # 保存拼接视频 (输入pha, fgr + 预测的com, bgr)
        #     modalities_to_cat = []
        #     if "com" in video_dict:
        #         modalities_to_cat.append(video_dict["com"] * 0.5 + 0.5)
        #     modalities_to_cat.append(inference_pha[0] * 0.5 + 0.5)
        #     modalities_to_cat.append(inference_fgr[0] * 0.5 + 0.5)
        #     if "bgr" in video_dict:
        #         modalities_to_cat.append(video_dict["bgr"] * 0.5 + 0.5)
            
        #     cat_video = torch.cat(modalities_to_cat, dim=-1)  # 在宽度维度拼接
        #     self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/BackgroundGeneration/cat_all_{count}.mp4")
        
        # ========== 4. Masking: 根据 pha 抠出前景 (输出 fgr) ==========
        # print("\n[Validation] Testing Masking mode...")
        # if inference_pha is not None and inference_fgr is not None:
        #     # 这里不需要 text 条件，训练时也是空 prompt
        #     inference_params = {
        #         "prompt": ["", "", "", ""],
        #         "negative_prompt":"色调艳丽，过曝，静态，细节模糊不清，字幕，风格，作品，画作，画面，静止，整体发灰，最差质量，低质量，JPEG压缩残留，丑陋的，残缺的，多余的手指，画得不好的手部，画得不好的脸部，畸形的，毁容的，形态畸形的肢体，手指融合，静止不动的画面，杂乱的背景，三条腿，背景人很多，倒着走",
        #         "seed": 1,
        ...
        #         # Masking 任务按目前 pipeline 设计只需要 pha/fgr 参与训练，
        #         # 这里作为可视化，直接把 GT com/pha/fgr/bgr 作为输入条件中的 fgr/pha 由数据集控制。
        #         "inference_com": inference_com,
        #         "inference_pha": inference_pha,
        #         "inference_fgr": None,
        ...
        #     video_dict = self.model.pipe(**inference_params)
        #     os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/Masking", exist_ok=True)

        #     # 保存预测 fgr
        #     if "fgr" in video_dict:
        #         fgr_gen = video_dict["fgr"]
        #         self._tensor2video(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/Masking/fgr_gen_{count}.mp4")

        #     # 拼接：输入 com/pha/fgr/bgr 如果有 + 预测 fgr
        #     modalities_to_cat = []
        #     if inference_com is not None:
        #         modalities_to_cat.append(inference_com[0] * 0.5 + 0.5)
        #     modalities_to_cat.append(inference_pha[0] * 0.5 + 0.5)
        #     if "fgr" in video_dict:
        #         modalities_to_cat.append(video_dict["fgr"] * 0.5 + 0.5)
        #     modalities_to_cat.append(inference_bgr[0] * 0.5 + 0.5)

        #     if modalities_to_cat:
        #         cat_video = torch.cat(modalities_to_cat, dim=-1)
        #         self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/Masking/cat_all_{count}.mp4")

        # ========== 5. ForegroundGeneration: 根据背景生成前景 (输出 com, pha, fgr) ==========
        print("\n[Validation] Testing ForegroundGeneration mode...")
        if inference_com is not None and inference_pha is not None and inference_fgr is not None:
            # 使用 gt bgr 作为条件，生成新的前景相关分支
            # 这里假设 val_batch 中存在 "bgr" 键
            inference_bgr = val_batch.get("bgr")
            if inference_bgr is not None:
                inference_params = {
                    "prompt": [
                        "一位身披黑色斗篷、头戴斗笠的冷艳女侠在雨中按剑而立。背景为：烟雨朦胧的江南水乡，青石板路蜿蜒，两侧是酒旗飘扬的客栈。",
                        "一位身披黑色斗篷、头戴斗笠的冷艳女侠在雨中按剑而立。背景为：烟雨朦胧的江南水乡，青石板路蜿蜒，两侧是酒旗飘扬的客栈。",
                        "一位身披黑色斗篷、头戴斗笠的冷艳女侠在雨中按剑而立。背景为：烟雨朦胧的江南水乡，青石板路蜿蜒，两侧是酒旗飘扬的客栈。",
                        "一位身披黑色斗篷、头戴斗笠的冷艳女侠在雨中按剑而立。背景为：烟雨朦胧的江南水乡，青石板路蜿蜒，两侧是酒旗飘扬的客栈。",
                    ],
                    ...
                    "tile_stride": [15, 26],
                    "is_inference": True,
                    "training_mode": "ForegroundGeneration",
                    "inference_com": None,
                    "inference_pha": None,
                    "inference_fgr": None,
                    "inference_bgr": inference_bgr,
                ...
                video_dict = self.model.pipe(**inference_params)
                os.makedirs(f"{self.output_path}/epoch_{self.epoch_id}/ForegroundGeneration", exist_ok=True)

                # 保存预测 com/pha/fgr
                if "com" in video_dict:
                    com_gen = video_dict["com"]
                    self._tensor2video(com_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/ForegroundGeneration/com_gen_{count}.mp4")
                if "pha" in video_dict:
                    pha_gen = video_dict["pha"]
                    self._tensor2video(pha_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/ForegroundGeneration/pha_gen_{count}.mp4")
                if "fgr" in video_dict:
                    fgr_gen = video_dict["fgr"]
                    self._tensor2video(fgr_gen * 0.5 + 0.5, f"{self.output_path}/epoch_{self.epoch_id}/ForegroundGeneration/fgr_gen_{count}.mp4")

                # 拼接：输入 bgr + 预测 com/pha/fgr
                modalities_to_cat = []
                if "com" in video_dict:
                    modalities_to_cat.append(video_dict["com"] * 0.5 + 0.5)
                if "pha" in video_dict:
                ...
                modalities_to_cat.append(inference_bgr[0] * 0.5 + 0.5)
                cat_video = torch.cat(modalities_to_cat, dim=-1)
                self._tensor2video(cat_video, f"{self.output_path}/epoch_{self.epoch_id}/ForegroundGeneration/cat_all_{count}.mp4")
