...
class ModelCheckpointCallback_wan2_1_fun_1_1_1_3b_control_normal_depth_train_v0(Callback):    
    """
    checkpoint callback
    """
...
class TensorboardLoggingCallback_wan2_1_fun_1_1_1_3b_control_normal_depth_train_v0(Callback):
...
class Trainer_wan2_1_fun_1_1_1_3b_control_normal_depth_train_v0:
    def __init__(
            self, 
            model, 
            optimizer, 
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for data in tqdm(self.val_dataloader, disable=not self.accelerator.is_main_process, desc=f"Epoch {self.epoch_id}: Validating"):
                ...
                inference_params["normals"] = data.get("normals", None)
                inference_params["disparities"] = data.get("disparities", None)
                ...
                inference_params["num_input_videos"] = 1
                # 执行推理
                try:
                    # rgbs process
                    ...
                    # disparity process
                    elif video_dict["context_index"].item() == 1:
